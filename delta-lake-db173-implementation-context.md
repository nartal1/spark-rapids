...
**DELETE/UPDATE branch:** 2026-05-12 (§0.K)
**DELETE/UPDATE PR-ready review:** 2026-05-13 (§0.K)
**Session:** Issues 1, 2, 6, 8, and 10 complete; Issue 3 DELETE/UPDATE is
implemented locally on `delta_db173_DELETE_UPDATE_support` and is PR-ready at
`c8a0f87b86c16ce02097ae3501c328f584bb720e`. Issue 6 final state is DBR-17.3
native cuDF-only GPU DV reads. The 2026-05-02 materialized DBR GPU DV reader was
an intermediate checkpoint and is not in the final squashed commit.
...
MERGE, OPTIMIZE, CDC + DV reads, and CI enablement remain follow-up work.
...
Jenkins Delta Lake test skip for `spark400db173` stays in place until the remaining
Delta follow-ups are resolved and Issue 7 removes it. Issue 3 is implemented locally,
Issue 6 is implemented in the native-DV branch, and Issue 8 is resolved as of
2026-04-24; see §0.D.
...
- `GpuDeleteCommand.scala` and `GpuUpdateCommand.scala` adapters/stubs depending
  on the active follow-on branch
- provider/probe files, local command-meta shims, and native DV read files

### 0.K DELETE/UPDATE Follow-on Branch Status (2026-05-12/13)
...
Branch split:
- Dropped the native-DV read commit `25d3e04c9` from DELETE/UPDATE branch history.
- Removed the two DV metric-scan hunks from the DELETE/UPDATE commit.
- Kept persistent deletion-vector write fallback in `DeleteCommandMetaShim.scala`
  and `UpdateCommandMetaShim.scala`.
- Native DV read files plus the metric-scan fallback are in the separate DV PR
  branch, squashed into `844678a6e`.

Current code shape:
- New shared files:
  `delta-lake/common/src/main/db-350db143-400db173/scala/com/databricks/sql/transaction/tahoe/rapids/GpuDeleteCommandBase.scala`
  and `GpuUpdateCommandBase.scala`.
- DB-17.3 local adapters:
  `GpuDeleteCommand.scala` and `GpuUpdateCommand.scala` extend the shared bases.
- DB-17.3 shims:
  `DeleteCommandMetaShim.scala` and `UpdateCommandMetaShim.scala` no longer
  unconditionally tag DELETE/UPDATE as unsupported. They tag CPU fallback only
  when persistent deletion-vector writes would be used.
- Row-tracking columns are preserved during DELETE and UPDATE rewrites.
- Whole-table and metadata-only DELETE use DBR-17.3's metadata-derived row-count
  path when `spark.databricks.delta.dmlMetricsFromMetadata.enabled` is enabled.
- Runtime guards throw if a persistent deletion-vector write reaches the GPU command
  despite the plan-time CPU fallback tag.
- New DBR-17.3 tests cover row tracking, whole-table DELETE metadata metrics,
  metadata-only DELETE metadata metrics, and persistent-DV write fallback behavior.
...
- Earlier broad validation: full `delta_lake_update_test.py` passed; full
  `delta_lake_delete_test.py` had failures only in native DV read tests that
  intentionally belong to the DV PR.
...
1 (scaffolding — **done**), 2 (writes — **done 2026-04-23**),
3 (DELETE/UPDATE — **PR-ready locally 2026-05-13**), 4-5 (MERGE/OPTIMIZE),
...
MERGE/OPTIMIZE still fall back to CPU via `willNotWorkOnGpu`; DELETE/UPDATE are
enabled on their follow-on branch except for persistent-DV write fallback.
