...
**Session:** Issue 1 (scaffolding) + Issue 2 (GPU writes) complete. DML, OPTIMIZE, DV
reads still on CPU.
...
- `DeltaSpark400DB173Provider.scala`, `DatabricksDeltaProviderBase.scala`,
  `RapidsDeltaUtils.scala`, all DML command files — unchanged.
...
Jenkins Delta Lake test skip for `spark400db173` stays in place until Issues 3-6, 8 are
resolved and Issue 7 removes it.
