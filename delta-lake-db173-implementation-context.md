...
**For resuming work across sessions — local note, do not commit in the PR**
...
**DB14.3/DB17.3 dedupe refactor:** 2026-05-06 (§0.G)
**Session:** Issue 1 (scaffolding + GPU writes) complete, slimmed, and deduped.
`delta_lake_write_test.py` and `delta_lake_test.py` are green on DB-17.3
(constraint-check tests pass; V1 `WriteIntoDeltaCommand` xfails under #11169). DML,
OPTIMIZE, DV reads still on CPU.
...
Originally implemented as three DB-17.3-only file changes; validated on cluster
(~3.5× speedup on a simple append, `test_delta_overwrite_by_expression_exec_v1` 4/4
passing, partitioned writes unblocked, Delta log parity confirmed). As of the
2026-05-06 refactor (§0.G), the shared parts of those changes live under
`common/src/main/db-350db143-400db173/scala`, with DB-17.3 retaining only the
...
**Current file shape after the 2026-05-06 refactor:**

1. Shared
   [GpuOptimisticTransactionWriteBase.scala](delta-lake/common/src/main/db-350db143-400db173/scala/com/databricks/sql/transaction/tahoe/rapids/GpuOptimisticTransactionWriteBase.scala)
   owns the common DB-14.3/DB-17.3 `gpuWriteFiles(inputData, writeOptions,
   additionalConstraints, isOptimizeOverride, ...)` pipeline.
2. DB-17.3
   [GpuOptimisticTransaction.scala](delta-lake/delta-spark400db173/src/main/scala/com/databricks/sql/transaction/tahoe/rapids/GpuOptimisticTransaction.scala):
...
3. [GpuDeltaFileFormatWriter.scala](delta-lake/delta-spark400db173/src/main/scala/com/databricks/sql/transaction/tahoe/rapids/GpuDeltaFileFormatWriter.scala)
   (new file):
...
4. Shared
   [GpuWriteIntoDeltaBase.scala](delta-lake/common/src/main/db-350db143-400db173/scala/com/databricks/sql/transaction/tahoe/rapids/GpuWriteIntoDeltaBase.scala)
   owns the common transaction/idempotency flow. DB-17.3
   [GpuWriteIntoDelta.scala](delta-lake/delta-spark400db173/src/main/scala/com/databricks/sql/transaction/tahoe/rapids/GpuWriteIntoDelta.scala):
...
**What remains version-specific** (explicit, to resist accidental broadening):

- `GpuOptimisticTransactionBase.scala` remains a version adapter. DB-17.3 owns the
  7-arg `writeFiles` safety-net override and the DB-17.3
  `DeltaInvariantCheckerExec(cpuPlan.session, cpuPlan, constraints)` constructor.
- DB-14.3 still owns `GpuDoAutoCompaction` registration through its
  `GpuOptimisticTransaction` adapter; DB-17.3 does not reference that class.
- `DatabricksDeltaProviderBase.scala`, `RapidsDeltaUtils.scala`, and DML command files
  are not broadened by this refactor.
...
### 0.G DB14.3/DB17.3 Dedupe Refactor (2026-05-06)

Reviewer feedback asked to apply the same "shared common base + thin version adapter"
trick used by the OSS `delta-33x-40x` code. The refactor was committed as:

```text
d8b1e4309 Refactor Databricks Delta transaction support
```

The shared implementation is **not** in broad
`delta-lake/common/src/main/databricks/scala`. It lives in a narrow source root included
only by DB-14.3 and DB-17.3:

```text
delta-lake/common/src/main/db-350db143-400db173/scala/
```

Only these POMs include that source root:
...
This avoids changing Databricks 12.2, 13.3, or other runtimes that already use
`common/src/main/databricks/scala`.

**Shared files extracted into the narrow root:**
...
**Version-specific adapters retained:**

- DB-14.3 keeps thin adapters for create-table, optimistic transaction base,
  optimistic transaction, and write-into-delta. The DB-14.3 adapter still owns the old
  `DeltaInvariantCheckerExec(cpuPlan, constraints)` constructor and the
  `GpuDoAutoCompaction` post-commit hook.
- DB-17.3 keeps thin adapters for create-table, optimistic transaction base,
  optimistic transaction, and write-into-delta. The DB-17.3 adapter still owns the
  `DeltaInvariantCheckerExec(cpuPlan.session, cpuPlan, constraints)` constructor, the
  `writeFilesAndGetExecutedPlan` / `writeFilesAndGetQueryExecution` overrides, the
  `RuntimeReplaceable` stats-expression normalization, the DB-17.3
  `GpuDeltaFileFormatWriter` path, and the `NoRowsCopiedTag` commit tag.
- `GpuDeltaCatalog`, `GpuDeltaDataSource`, and the copyright-only identical shim utilities
  are now shared. `GpuDeltaDataSource` uses the Scala-2.13-friendly
  `parameters.filter { case (key, _) => ... }` form.

**Line-count shape after the refactor:**

- Refactor commit: `29 files changed, 1335 insertions(+), 2274 deletions(-)`.
- DB-17.3 module Scala LOC after the final dedupe pass: 983 raw LOC, 602
  non-comment/nonblank LOC.
- GitHub still counts the narrow shared files as PR additions because they are new
  relative to `main`. The `~700` number is the effective DB17-specific code size, not
  GitHub additions.

**Verification on the DB-17.3 cluster:**
...
bash integration_tests/run_pyspark_from_build.sh \
  ...
  -k delta_lake_write_test.py
bash integration_tests/run_pyspark_from_build.sh \
  --runtime_env=databricks --delta_lake --test_type=$TEST_TYPE \
  -k delta_lake_test.py
```

Results after the final amend:

- `delta_lake_write_test.py`: 97 passed, 2 skipped, 26 xfailed, 4 xpassed.
- `delta_lake_test.py`: 178 passed, 42 skipped, 13 xpassed.

This machine can only build the DB-17.3 runtime. Other Databricks versions were checked
by code review only; pre-merge CI must validate them.
...
1 (scaffolding — **done**), 2 (writes — **done 2026-04-23**), 3-5 (DML), 6 (DV reads),
8 (GPU Delta read relative-path bug), 9 (identity columns), 7 (CI enablement).

Issue 1 deliverable (initially done in `bd0f6e5f`, refactored in `d8b1e4309`): Create
`delta-lake/delta-spark400db173/` and share the large DB-14.3/DB-17.3 overlap through
`delta-lake/common/src/main/db-350db143-400db173/scala`. All Spark 4.0 / DB-17.3 API
migrations are applied; all Delta DML paths fall back to CPU via `willNotWorkOnGpu`.
...
│   │                            # Narrow shared root for DB-14.3 and DB-17.3 only
│   └── delta-33x-40x/scala/    # Shared for OSS Delta 3.3+/4.0+ (DV common code)
├── delta-spark350db143/         # DB-14.3
├── delta-spark400db173/         # DB-17.3 module
└── delta-stub/                  # No-op fallback for unsupported profiles
```
