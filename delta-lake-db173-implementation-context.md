...
**Session:** Issues 1, 2, 6, 8, and 10 complete. Issue 6 now includes both the
2026-05-02 materialized DV reader and the 2026-05-05 native cuDF DV follow-up.
`delta_lake_write_test.py`, `delta_lake_test.py`, and `delta_lake_delete_test.py`
are green on DB-17.3 for the validated local runs. DML (DELETE/UPDATE/MERGE),
OPTIMIZE, CDC + DV reads, and CI enablement remain follow-up work.
...
- The V1 path still handles COALESCING by falling through to the multithreaded
  DV reader path. The native follow-up adds the COALESCING native cuDF path.
...
- Native DV scanning is selected only when DBR metadata-row-index mode and
  ...
  true. Otherwise DBR-17.3 uses the materialized Scala/GPU DV reader.
