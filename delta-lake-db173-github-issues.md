...
- **2026-05-06 (Issue 6 native-only cleanup):** Team review narrowed DBR-17.3 GPU
  deletion-vector reads to the native cuDF path only. If
  `spark.databricks.delta.deletionVectors.useMetadataRowIndex` or
  `spark.rapids.sql.delta.deletionVectors.predicatePushdown.enabled` is false for a
  DBR-17.3 DV scan, the scan falls back to CPU. The intermediate DBR materialized GPU
  files `GpuDeltaParquetFileFormatDV.scala` and `RapidsRowIndexFilters.scala` were
  removed before squashing. OSS Delta materialized DV support remains separate.
- **2026-05-06 (shared transaction refactor):** DB-14.3 and DB-17.3 now share the
  overlapping transaction/catalog/data-source/shim code through
  `delta-lake/common/src/main/db-350db143-400db173/scala`. `delta-spark400db173`
  keeps version-specific adapters, DML CPU-fallback stubs, `GpuDeltaFileFormatWriter`,
  provider/probe files, and native DV read files.
- **2026-05-11 (PR review-comment sync + DV fallback hardening):** The follow-on
  DV branch carries review-comment commit `362e9d346` below the amended native-only
  DV commit. It clears `pathPrefix` after absolute-file expansion in
  `FilePartitionShims.copyWithFiles`, records the wide-schema follow-up as #14775,
  hardens CTAS/RTAS conversion fallback, runs Databricks `AutoCompact` as a CPU
  post-commit hook, and narrows `GpuCheckDeltaInvariant` conversion to
  `check.child`. Native DBR DV scans now also fall back to CPU for row-index filter
  semantics other than `IF_CONTAINED`, including the `IF_NOT_CONTAINED` shape seen
  in merge validation.
- **2026-05-14 (DML skip-row pushdown guard):** Native DBR DV predicate pushdown now
  drops/prunes the skip-row predicate only when the child plan contains a native GPU DV
  scan (`GpuDeltaParquetFileFormatNativeDV`). Plugin-enabled DBR DELETE/DML fallback
  plans keep DB's skip-row filter, fixing the
  `DELTA_DELETION_VECTOR_CARDINALITY_MISMATCH` regression in
  `test_delta_delete_twice_with_dv`. This confirms DBR-17.3 support is GPU DV reads,
  not GPU DELETE with persistent deletion vectors. The same review pass added
  `test_db173_missing_row_index_filter_assertion_guard`, a Python canary that fails if
  DB changes the missing-row-index-filter assertion wording so RAPIDS'
  `MISSING_ROW_INDEX_FILTER_MESSAGE` no longer matches.
...
Historically this scaffold started as 22 DB-17.3-local Scala files. After the
2026-05-06 shared transaction refactor, the DB-17.3 module has 16 local Scala
files plus one Java helper, and both DB-14.3 and DB-17.3 consume the new shared
`common/src/main/db-350db143-400db173/scala` root. The command classes still
must be visible either as module-local adapters/stubs or via the shared root
because `DatabricksDeltaProviderBase` registers metas for those command types.

### Scope

**DB-17.3 module:** `delta-lake/delta-spark400db173/` (after 2026-05-06 refactor)
- `pom.xml`
- Native DV Java helper: `DeltaBitmapUtils`
- 5 provider/read files (`DeltaProbe`, `DeltaSpark400DB173Provider`,
  `GpuDeltaParquetFileFormat`, `GpuDeltaParquetFileFormatNativeDV`,
  `RapidsDeletionVectors`)
- 4 local shim files (`DeleteCommandMetaShim`, `UpdateCommandMetaShim`,
  `MergeIntoCommandMetaShim`, `ShimShuffledRowRDD`)
- Local write-path command/transaction adapters (live on GPU):
  `GpuOptimisticTransactionBase`, `GpuOptimisticTransaction`, `GpuWriteIntoDelta`,
  `GpuDeltaFileFormatWriter`, `GpuCreateDeltaTableCommand`
- Shared DB-14.3/DB-17.3 root:
  `GpuCreateDeltaTableCommandBase`, `GpuDeltaCatalog`, `GpuDeltaCatalogCommon`,
  `GpuOptimisticTransactionBaseCommon`, `GpuOptimisticTransactionWriteBase`,
  `GpuWriteIntoDeltaBase`, `GpuDeltaDataSource`, `DeltaLogShim`,
  `InvariantViolationExceptionShim`, `MetadataShims`, `ShimDeltaUDF`
...
- `delta-spark350db143` and `delta-spark400db173` poms add
...
- Historical Issue 1 behavior: `GpuDeltaParquetFileFormat` blocked GPU for any DV table.
  Issue 6 replaced this with the native cuDF-only DBR-17.3 DV read path described below.
...
**Status:** Done in PR #14810 / base `2ba5bc25e`. DELETE/UPDATE with persistent
DVs still falls back to CPU as documented below.
...
The 2026-05-14 `test_delta_delete_twice_with_dv` regression validated this boundary:
the DELETE command remained CPU fallback, and the fix was to keep the DBR skip-row
filter in that fallback DML plan rather than attempting to run DELETE with DVs on GPU.
...
**Status (updated 2026-05-21):** PR #14847 is in review on
`delta_db173_optimize_auto_compaction` at `f8525abf7`. The smaller regular
OPTIMIZE + inline auto-compaction scope has passed the requested DBR 17.3 build
and focused tests. Liquid clustering remains a separate follow-up.

Current smaller PR commit stack after May 21 review hardening:

- `37903ee00` - Add DBR 17.3 Delta OPTIMIZE support
- `704568255` - Add DBR 17.3 Delta auto compaction support
- `bacb9e728` - Fix DBR 17.3 Delta OPTIMIZE tests
- `15c3701e3` - addressed review comments in Scala OPTIMIZE code
- `fcf62bc5c` - fix license header and integration tests
- `f6bd6eb34` - add scoped GPU OPTIMIZE execution assertion
- `f8525abf7` - isolate OPTIMIZE parity tests from auto-compact defaults

Liquid follow-up commits on `delta_db173_OPTIMIZE_AUTO_COMPACT`:
...
- PR 1: use `delta_db173_optimize_auto_compaction` at `f8525abf7` for
  DBR 17.3 regular OPTIMIZE and auto-compaction. Use `Refs #14599`, not
  `Closes #14599`, unless the issue is split.
- PR 2: include `0141241f0` and `a05033865` for liquid clustered OPTIMIZE,
  `OPTIMIZE FULL`, and general liquid clustered write-path completion.

### Description

Enable GPU-accelerated OPTIMIZE command and auto-compaction post-commit hook for
Delta Lake on Databricks 17.3. Liquid clustering is related because liquid
clustering is physically maintained through OPTIMIZE, but it is complex enough to
remain a separate follow-up PR.

### Delivered / Local Scope

- Add DBR 17.3 command wiring for `OptimizeTableCommand` and
  `OptimizeTableCommandEdge`.
- Add DBR 17.3 `GpuOptimizeTableCommand`, `GpuOptimizeTableCommandEdge`, and
  `GpuOptimizeExecutor`.
- Support regular unpartitioned and partitioned OPTIMIZE.
- Keep unsupported cases on CPU with explicit tagging: writable deletion-vector
  tables, ZORDER, REORG, and deletion-vector cleanup.
- Add DBR 17.3 GPU auto-compaction using a `GpuAutoCompact` post-commit hook and
  the DBR 17.3 `CommittedTransaction` API.
- Reuse `AutoCompactUtils.prepareAutoCompactRequest` and preserve CPU fallback
  for background auto-compaction and writable deletion-vector tables.
- Thread `catalogTable` through DBR 14.3/17.3 GPU optimistic transactions for
  commit routing used by DBR 17.3 OPTIMIZE.
- Add a runtime writable-DV guard to the DBR 17.3 optimize executor, preserve
  DBR row-tracking commitInfo tag parity, close the inline auto-compaction
  observer in the `finally` path, and match DBR's auto-compaction min-file-size
  default when the min config is unset.
- Follow-up liquid commits add clustered OPTIMIZE / `OPTIMIZE FULL` and liquid
  write-path metadata handling.

### Tests

Run on a DBR 17.3 cluster for the smaller PR:

- `delta_lake_optimize_table_test.py`
- `delta_lake_auto_compact_test.py`

Latest smaller-PR validation on 2026-05-21:

- `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh` -> `BUILD SUCCESS`
- Focused OPTIMIZE + auto-compact run -> `19 passed`, `28 warnings`
- Focused OPTIMIZE run after adding scoped `GpuExecutedCommandExec` assertion -> `5 passed`, `28 warnings`
- Focused OPTIMIZE run after disabling auto-compact in the OPTIMIZE parity config -> `5 passed`, `28 warnings`
- `git diff --check` passed

Run for the liquid clustering follow-up:

- `delta_lake_liquid_clustering_test.py`
- relevant `delta_lake_optimize_table_test.py` clustered-table selections
...
**Status:** Done locally and squashed as native-only DBR-17.3 GPU DV support in
`ada2580ea3ff0c558f62b1e503a0f302343e83f9` (`[databricks] Add native-only
Delta DV reads for DBR 17.3`). The 2026-05-02 materialized DBR GPU path was an
intermediate checkpoint only; the final DBR-17.3 GPU path is native cuDF DV
scanning, with CPU fallback when the native gates are disabled or the DBR
row-index filter type is not `IF_CONTAINED`. Issue 6 validated the targeted
Delta read/delete, native footer, count-star, scan-split, merge-regression, and
auto-compact selections on a DB-17.3 cluster. The 2026-05-14 follow-up also
guards skip-row predicate removal so it applies only to native GPU DV scans and
does not alter CPU-fallback DELETE/DML bitmap-writing plans. See
...
**DB-17.3 final native-only DV read path:**
- `DeltaSpark400DB173Provider.scala` recognizes DBR DV scan shapes, gates native DV
  pushdown on DBR metadata-row-index mode plus RAPIDS DV predicate pushdown, and tags
  DBR DV scans for CPU fallback when either native gate is false.
- `GpuDeltaParquetFileFormat.scala` mirrors the DBR CPU file-format flags, preserves the
  non-DV GPU Delta read behavior, and participates in disabled-gate and unsupported
  row-index-filter-type DV fallback tagging.
- `GpuDeltaParquetFileFormatNativeDV.scala` adds native cuDF DV scans for PERFILE,
  MULTITHREADED, COALESCING, native footer, and zero-column/count-star paths.
- `RapidsDeletionVectors.scala` supports DBR descriptor/provider lookup through
  `PartitionedFile`, `RowIndexFilterProvider`, and `TahoeFileIndex`, column-mapping
  filter translation, and native bitmap/row-count helpers.
- `DeltaBitmapUtils.java` converts DBR `SerializedBitmap` data to cuDF-compatible
  standard portable roaring bitmap bytes.
- `delta_lake_test.py` and `delta_lake_delete_test.py` assert the native-gated DBR
  behavior and CPU fallback when the gates are disabled.
- `DeltaSpark400DB173Provider.scala` keeps DBR skip-row predicate removal scoped to
  child plans that contain `GpuDeltaParquetFileFormatNativeDV`; CPU-fallback DML
  plans keep the predicate.
- The intermediate DBR materialized GPU files `GpuDeltaParquetFileFormatDV.scala` and
  `RapidsRowIndexFilters.scala` are removed from the final squashed commit.
...
- Native support for DBR row-index filter semantics other than `IF_CONTAINED`,
  or continued CPU fallback for those plan shapes.
...
- GPU DELETE/UPDATE with persistent deletion vectors remains part of the DML follow-up
  work and is not delivered by Issue 6.
...
- Targeted DBR Delta DV read slice 1 passed: 80 selected, 80 passed.
  Covered PERFILE, MULTITHREADED, COALESCING, mixed DV/no-DV, scan splitting,
  native footer, and disabled native-gate CPU fallback.
- Targeted DBR Delta DV read slice 2 passed: 120 selected, 108 passed,
  12 expected Databricks skips. Covered empty DVs, multithreaded reads,
  native-footer count-star, coalescing count-star, and multithreaded combined
  count-star selection.
- 2026-05-11 focused DV selection passed: 194 tests, 0 failures, 0 errors.
- 2026-05-11 targeted merge regression passed: `test_delta_merge_query`,
  1 test, 0 failures.
- 2026-05-11 broader Delta read/delete selection passed: 289 tests,
  0 failures, 0 errors, 40 skipped.
- 2026-05-11 `delta_lake_auto_compact_test.py` selection passed: 14 tests,
  0 failures.
- 2026-05-14 build passed again:
  `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh`.
...
- [FilePartitionShims.scala](sql-plugin/src/main/spark400db173/scala/org/apache/spark/sql/execution/rapids/shims/FilePartitionShims.scala) —
  ...
  `FilePartition.getFilePartitions`. No-op on all other shims. The 2026-05-11
  review-comment sync also clears `pathPrefix` in `copyWithFiles` after storing
  already-absolute files, avoiding any later dependency on `Path.isAbsolute()`
  behavior.
...
## Dependency Graph (updated 2026-05-14 after Issue 6 skip-row hardening)
...
Issue 1: Scaffold delta-spark400db173 (module + CPU fallbacks)   ← DONE
  ...
  +---> Issue 2: GPU Delta writes                                ← DONE (2026-04-23)
          ...
          |
          +---> Issue 6: GPU Deletion Vector reads                  ← DONE (native-only, hardened 2026-05-14)
          |
...
for read-heavy workloads and does not require GPU DML to create the DVs. The
final Issue 6 DBR-17.3 GPU scope is native cuDF DV reads only; disabled native
gates fall back to CPU. Issues 8 and 10 are resolved. Issue 7 lands last once
the remaining test matrix is clean. `delta_lake_write_test.py` and the targeted
DV slices from `delta_lake_test.py` / `delta_lake_delete_test.py` are green in
the local DB-17.3 validation runs noted above. The Group-A V1
