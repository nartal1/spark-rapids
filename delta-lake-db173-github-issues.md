...
- **2026-05-12/13 (Issue 3 DELETE/UPDATE branch split + PR-ready review):**
  `delta_db173_DELETE_UPDATE_support` was cleaned to drop the native-DV read commit
  (`25d3e04c9`) and keep DELETE/UPDATE as its own follow-on PR. Final reviewed HEAD:
  `c8a0f87b86c16ce02097ae3501c328f584bb720e Support Update+DELETE for DB-17.3`.
  Native DV read files and the DV metric-scan fallback live in the separate DV PR
  branch; the metric-scan fallback is squashed into
...
At Issue 2 time, DELETE, UPDATE, MERGE, OPTIMIZE (Issues 3/4/5), and DV reads
(Issue 6) still fell back to CPU. Current follow-on status: Issue 3 is
implemented locally on `delta_db173_DELETE_UPDATE_support`, Issue 6 is
implemented in the native-DV branch, and MERGE/OPTIMIZE remain CPU fallback.
Writes to identity-column tables currently block `ProjectExec` to CPU via
`GenerateIdentityValues` (Issue 9).
...
**Status:** PR-ready locally on `delta_db173_DELETE_UPDATE_support` as
`c8a0f87b86c16ce02097ae3501c328f584bb720e Support Update+DELETE for DB-17.3`.
The branch intentionally excludes native DV read support and DV metric-scan fallback
code, which are owned by the separate DV PR. DELETE/UPDATE paths that would write
persistent deletion vectors still fall back to CPU.

### Description

Enable GPU-accelerated DELETE and UPDATE commands for Delta Lake on Databricks 17.3.
After the 2026-04-24 slim-down, `GpuDeleteCommand` / `GpuUpdateCommand` existed
only as ~30-line stubs (compile-only; `run()` throws). The Issue 3 branch
replaces those stubs with DB-17.3 adapters over shared DB-14.3/DB-17.3 command
base implementations.

### Scope

- Add `GpuDeleteCommandBase.scala` under
  `common/src/main/db-350db143-400db173/scala` for touched-files discovery,
  rewrite, DMLUtils tagging, metrics, and CDF handling.
- Add `GpuUpdateCommandBase.scala` under the same shared root for UPDATE file
  discovery, rewrite, metrics, CDF handling, and Spark 4.0 API adapters.
- Update DB-17.3 `GpuDeleteCommand.scala` and `GpuUpdateCommand.scala` to extend
  those shared bases instead of throwing from stubs.
- Update `DeleteCommandMetaShim.scala` and `UpdateCommandMetaShim.scala` to remove
  the unconditional DB-17.3 fallback while preserving CPU fallback for persistent
  deletion-vector writes.
- Preserve Delta row-tracking columns during DELETE and UPDATE rewrites.
- Report metadata-derived affected row counts for whole-table and metadata-only
  DELETE when `spark.databricks.delta.dmlMetricsFromMetadata.enabled` is enabled.
- Keep runtime guards as a defensive backstop if a persistent deletion-vector write
  reaches the GPU command despite plan-time CPU fallback.
- Keep MERGE and OPTIMIZE out of this PR.
...
Validated locally on DBR 17.3:
- Build: `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh` passed.
- Focused DBR-17.3 DELETE/UPDATE regressions passed:
  `test_delta_delete_entire_table_reports_row_count_db173`,
  `test_delta_delete_metadata_only_reports_row_count_db173`,
  `test_delta_delete_preserves_row_tracking_db173`, and
  `test_delta_update_preserves_row_tracking_db173`.
- Focused persistent-DV write fallback tests passed:
  `test_delta_deletion_vector_fallback` and
  `test_delta_update_fallback_with_deletion_vectors`.
- Earlier broad validation: full `delta_lake_update_test.py` passed; full
  `delta_lake_delete_test.py` had failures only in `test_delta_deletion_vector_read*`.
  Those native DV read tests are intentionally covered by Issue 6 / the DV PR branch,
  not this DELETE/UPDATE branch.
...
`844678a6ebc9cdc7ca02852b0ccd2999ec8e4c46` (`[databricks] Add native-only
...
scanning, with CPU fallback when the native gates are disabled. Issue 6
validated the targeted Delta read/delete, native footer, count-star, and
scan-split suites on a DB-17.3 cluster. See
...
- `GpuDeltaParquetFileFormat.scala` mirrors the DBR CPU file-format flags, preserves the
  non-DV GPU Delta read behavior, and participates in disabled-gate DV fallback tagging.
...
## Dependency Graph (updated 2026-05-13 after Issue 3 PR-ready review)
...
Issue 1: Scaffold delta-spark400db173 (module + CPU fallbacks)   ← DONE
  ...
  +---> Issue 2: GPU Delta writes                                ← DONE (2026-04-23)
          ...
          |
          +---> Issue 3: GPU DELETE + UPDATE                    ← PR-ready locally (2026-05-13)
          |
          +---> Issue 4: GPU MERGE INTO
          |
          +---> Issue 5: GPU OPTIMIZE + auto-compaction
          |
          +---> Issue 6: GPU Deletion Vector reads                  ← DONE (native-only, 2026-05-06)
          |
...
Issues 4 and 5 can still be developed in parallel. Issue 3 is implemented and
PR-ready on the DELETE/UPDATE branch at
`c8a0f87b86c16ce02097ae3501c328f584bb720e`; it is independent of the native-DV
read PR except for small textual overlap in DBR-17.3 Delta files if branches are
merged in opposite orders. Issue 6 was delivered
...
the remaining test matrix is clean. `delta_lake_write_test.py`, the targeted
DV slices from `delta_lake_test.py` / `delta_lake_delete_test.py`, and the
targeted DELETE/UPDATE slice are green in the local DB-17.3 validation runs
noted above. The Group-A V1 `WriteIntoDeltaCommand` GPU acceleration is a
separate quality-of-coverage follow-up tracked under #11169.
