...
- **2026-05-02 (Issue 6 implemented):** DB-17.3 GPU deletion-vector reads now use the
  ...
  `delta_lake_delete_test.py` were green in local DB-17.3 validation. Native cuDF DV
  reading, DV predicate pushdown, split/native optimized DV scanning, true DV-aware
  coalescing, and CDC + DV reads remain follow-up work.
...
**Status:** Done locally on 2026-05-02. Issue 6 landed the DB-17.3
V1/materialized DV read path and validated the targeted Delta read/delete suites
on a DB-17.3 cluster. See
...
- `DeltaSpark400DB173Provider` now recognizes the DB-17.3 GPU Delta format, detects
  DV scans, prunes metadata columns, and keeps `canPushDVPredicateDownToScan=false`.
...
- Split/native optimized DV scanning
