...
- Restore `registerPostCommitHook(GpuDoAutoCompaction)` in
  `GpuOptimisticTransaction.gpuWriteFiles` (replacing the current Databricks
  `AutoCompact` CPU hook when GPU auto-compaction is supported, with the
  `autoCompactEnabled` guard reading `DeltaSQLConf.DELTA_AUTO_COMPACT_ENABLED` /
