...
- **2026-04-30 (PR review hardening):** Static review against the live DB-17.3 JARs
  ...
  `col(name)`.
- **2026-05-06 (review dedupe refactor):** The large DB-14.3/DB-17.3 overlap was moved
  into the narrow shared root
  `delta-lake/common/src/main/db-350db143-400db173/scala`, included only by
  `delta-spark350db143` and `delta-spark400db173` (plus generated Scala-2.13 mirrors).
  Shared files now cover create-table, catalog, optimistic transaction base/write
  plumbing, write-into-delta flow, `GpuDeltaDataSource`, and the copyright-only identical
  shim utilities; version modules retain thin adapters for API differences. Commit
  `d8b1e4309` is `1335 insertions(+), 2274 deletions(-)`. DB17-specific Scala size is
  now 983 raw LOC / 602 non-comment LOC.
...
- 4 DB17-specific shim files (`DeleteCommandMetaShim`, `UpdateCommandMetaShim`,
  `MergeIntoCommandMetaShim`, `ShimShuffledRowRDD`); `DeltaLogShim`, `MetadataShims`,
  `ShimDeltaUDF`, and `InvariantViolationExceptionShim` are shared with DB14.3.
- Write-path command/transaction files (live on GPU; many are thin adapters after the
  2026-05-06 dedupe refactor):
...
- Shared DB14.3/DB17.3 write/catalog/data-source code:
...
**Latest DB-17.3 local verification after the 2026-05-06 refactor:**
`WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh`, `delta_lake_write_test.py`
(97 passed, 2 skipped, 26 xfailed, 4 xpassed), and `delta_lake_test.py`
(178 passed, 42 skipped, 13 xpassed).
...
**Current implementation shape after the 2026-05-06 refactor:** the behavior below is
still the delivered Issue 2 behavior, but much of the implementation now lives in the
narrow shared DB14.3/DB17.3 source root. DB-17.3 keeps thin adapters for API differences;
older Databricks modules other than DB-14.3 are not touched by that root.
...
`GpuOptimisticTransaction.scala` now extends shared
`GpuOptimisticTransactionWriteBase`, which owns the legacy GPU write pipeline
(`RapidsDeltaWrite` stub + file writer + Delta stats + identity-column stats).
The DB-17.3 adapter adds three entry points:
...
It also routes the actual write through `GpuDeltaFileFormatWriter.write(...)` instead
of `GpuFileFormatWriter.write(...)`.
...
`GpuWriteIntoDelta.scala` now extends shared `GpuWriteIntoDeltaBase`. The DB-17.3
adapter stamps `NoRowsCopiedTag = true` on the commit (plain writes copy no rows; skip
when `replaceWhere` is set). It uses the existing
`DMLUtils.TaggedCommitData(actions).withTag(...)` → `.stringTags` → 3-arg
`txn.commit(actions, op, tags)` pattern from `GpuDeleteCommand`. Needed for Delta-log
parity with CPU — CPU's `writeFilesAndGetExecutedPlan` default body sets this tag via a
private `recordWriteFilesOperation` helper, which our override bypasses.
...
## Dependency Graph (updated 2026-04-28 after Issue 10 resolution)
...
Issues 3, 4, 5 can be developed in parallel. Issue 6 depends on all DML paths
working for full DV test coverage. Issues 8 and 10 are resolved. Issue 7 lands last
once the remaining tests pass — `delta_lake_write_test.py` is now green on DB-17.3
(Issue 10 was the last hard blocker for it). The Group-A V1 `WriteIntoDeltaCommand`
GPU acceleration is a separate quality-of-coverage follow-up tracked under #11169.
