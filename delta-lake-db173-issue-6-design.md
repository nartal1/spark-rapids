...
2026-05-12/13 branch-split amend: the native-DV PR also owns the DBR 17.3
metric-scan fallback for DELETE/UPDATE-shaped DV skip-row filters. That logic
was removed from the separate DELETE/UPDATE branch and squashed into the native
DV commit so DV scan safety remains with Issue 6. The separate Issue 3
DELETE/UPDATE branch is PR-ready at
`c8a0f87b86c16ce02097ae3501c328f584bb720e`; it supports non-persistent-DV
DELETE/UPDATE row rewrites on GPU and keeps persistent DV writes on CPU.
...
- GPU MERGE/OPTIMIZE or persistent DV writes. Issue 3 DELETE/UPDATE is covered
  by the separate PR-ready branch and is not part of this Issue 6 DV-read design.
- CDC + DV reads.
