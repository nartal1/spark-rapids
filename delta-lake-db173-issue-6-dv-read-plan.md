...
**Status:** **Implemented and validated locally**. The initial V1/materialized
DV reader landed as an intermediate checkpoint on 2026-05-02; the native cuDF
DV follow-up landed locally on 2026-05-05; the final DBR-17.3 native-only
...
skip-row predicate removal in plugin-enabled DML fallback plans plus a Python
canary for DB missing-row-index-filter assertion wording.
...
## 2026-05-06 Final Native-only Status
...
The final DBR-17.3 GPU DV read path is **native cuDF only**. The earlier
V1/materialized DBR GPU reader was removed after team review. If a DBR-17.3 DV
scan does not satisfy the native gates, the scan falls back to CPU. This is a
DBR-17.3-specific scope decision and does not remove the OSS Delta
materialized/non-cuDF DV path.
...
`DeltaSQLConf.DELETION_VECTORS_USE_METADATA_ROW_INDEX`. When it is explicitly
false, or when RAPIDS DV predicate pushdown is false, DBR-17.3 DV scans are
tagged for CPU fallback.

2026-05-11 hardening: DBR row-index filters whose type is not
`RowIndexFilterType.IF_CONTAINED` are also tagged for CPU fallback. This was
added after `test_delta_merge_query` surfaced an `IF_NOT_CONTAINED` provider; the
native cuDF DV API applies bitmaps as rows to drop, while `IF_NOT_CONTAINED`
means keep marked rows.

2026-05-14 hardening: DBR skip-row predicates are removed/pruned only when the
child plan contains a `GpuFileSourceScanExec` backed by
`GpuDeltaParquetFileFormatNativeDV`. This prevents native-read pushdown from
rewriting DBR DELETE/DML bitmap-writing plans that are still CPU fallback and
must keep DB's skip-row filter for Delta's DV cardinality check.

Final implementation scope:
- `DeltaSpark400DB173Provider.scala` recognizes DBR DV scan shapes, gates native
  pushdown, selects the native format when the gates are true, and tags DV scans
  for CPU fallback when the gates are false. Its skip-row predicate removal is
  additionally guarded by the presence of a native GPU DV scan in the child plan.
- `GpuDeltaParquetFileFormat.scala` preserves non-DV GPU Delta reads, mirrors the
  DBR CPU file-format flags, handles column mapping/filter translation for
  non-DV reads, and participates in disabled-gate and unsupported-filter-type DV
  fallback tagging.
- `GpuDeltaParquetFileFormatNativeDV.scala` implements native cuDF DV reads for
  PERFILE, MULTITHREADED, COALESCING, native-footer, and zero-column/count-star
  paths.
- `RapidsDeletionVectors.scala` handles DBR descriptor/provider lookup through
  `PartitionedFile`, `RowIndexFilterProvider`, and `TahoeFileIndex`, DBR bitmap
  loading/row-count bookkeeping, row-group metadata helpers, and column-mapping
  filter translation.
- `DeltaBitmapUtils.java` bridges DBR package-private bitmap serialization APIs
  so DBR `SerializedBitmap` data becomes cuDF-compatible standard portable
  roaring bitmap bytes.
- `delta_lake_test.py` and `delta_lake_delete_test.py` cover the native-gated
  DBR behavior and disabled-gate CPU fallback.

Removed from the final squashed commit:
- `GpuDeltaParquetFileFormatDV.scala`
- `RapidsRowIndexFilters.scala`

Validation:
- Build: `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh` passed.
- Targeted DBR Delta DV slice 1: 80 selected, 80 passed.
- Targeted DBR Delta DV slice 2: 120 selected, 108 passed, 12 expected
  Databricks skips.
- Combined directly affected DBR DV read surface: 200 selected; 188 passed,
  12 expected Databricks skips.
- 2026-05-11 focused DV selection: 194 tests, 0 failures, 0 errors.
- 2026-05-11 targeted merge regression: `test_delta_merge_query`, 1 test,
  0 failures.
- 2026-05-11 broader Delta read/delete selection:
  `delta_lake_test.py or delta_lake_delete_test.py`, 289 tests, 0 failures,
  0 errors, 40 skips.
- 2026-05-11 auto-compact selection: `delta_lake_auto_compact_test.py`,
  14 tests, 0 failures.
- 2026-05-14 build:
  `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh` passed.
- 2026-05-14 targeted DML fallback regression:
  `delta_lake_delete_test.py::test_delta_delete_twice_with_dv` passed with
  `SPARK_RAPIDS_TEST_DATAGEN_SEED=1778719081` and OOM injection enabled.
- 2026-05-14 assertion-message canary:
  `delta_lake_test.py::test_db173_missing_row_index_filter_assertion_guard`
  protects RAPIDS' `MISSING_ROW_INDEX_FILTER_MESSAGE` against DB wording drift.

## 2026-05-02 Implementation Status (historical checkpoint)
...
cuDF/Base2 path. This was an intermediate checkpoint only; the final 2026-05-06
squashed commit removes this DBR materialized GPU path.
...
- Updated `DeltaSpark400DB173Provider.scala` for GPU Delta format recognition,
  ...
  `canPushDVPredicateDownToScan=false` for the initial V1 path.
...
The native-reader gaps listed in the original blueprint were closed by the
2026-05-05 follow-up described below, then the DBR materialized GPU path was
removed by the 2026-05-06 native-only cleanup. CDC + DV reads and Issue 7 CI
enablement remain open.

## 2026-05-05 Native cuDF DV Follow-up Status

The DBR-17.3 native DV follow-up is implemented locally. This supersedes older
parts of this source-of-truth document that say native cuDF DV, DV predicate
pushdown, split/native optimized scanning, true coalescing, or native-footer DV
tests are future work.

Implementation scope:
- `DeltaSpark400DB173Provider.scala` enables native DV predicate pushdown only
  when DBR metadata-row-index mode and
  `spark.rapids.sql.delta.deletionVectors.predicatePushdown.enabled` are both
  true.
- `GpuDeltaParquetFileFormatNativeDV.scala` implements native cuDF DV scans for
  PERFILE, MULTITHREADED, and COALESCING.
- `RapidsDeletionVectors.scala` recovers DBR DV descriptors/providers from
  `PartitionedFile`, `RowIndexFilterProvider`, or `TahoeFileIndex`.
- `DeltaBitmapUtils.java` converts DBR `SerializedBitmap` data to
  cuDF-compatible standard portable roaring bitmap bytes.
- `delta_lake_test.py` includes the native footer, count-star, and scan-split
  validation used for this follow-up.

Validation completed:
- DBR-17.3 dist build passed:
...
Known DBR-specific caveats:
- Predicate pushdown is intentionally narrow. It recognizes current DBR-17.3
  skip-row predicate shapes for `__delta_internal_is_row_deleted` and
  `_databricks_internal_edge_computed_column_skip_row`; it is not a general
  arbitrary-expression pushdown framework.
- Native scanning requires DBR metadata-row-index mode plus the RAPIDS DV
...
- Only `RowIndexFilterType.IF_CONTAINED` is supported on the native path. DBR
  row-index filters with other semantics, including `IF_NOT_CONTAINED`, fall
  back to CPU.
...
- The DBR native path uses DBR `DeletionVectorStoreEdge` / `SerializedBitmap`
  APIs and `DeltaBitmapUtils.java`; it does not use the OSS
  `RapidsDeletionVectorStore` / `RapidsFileIO` path.

---

## Historical Blueprint Notes

The sections below preserve the implementation blueprint and review context that
led to the final code. Any references below to adding `GpuDeltaParquetFileFormatDV.scala`,
adding `RapidsRowIndexFilters.scala`, using `canPushDVPredicateDownToScan=false`, or
falling back to a DBR materialized Scala/GPU DV reader describe the superseded
2026-05-02 checkpoint, not the final 2026-05-06 squashed commit.
...
This plan originally covered only the **V1 read path**: GPU Parquet read followed by a
post-read `is_row_deleted` mask materialized from the Delta deletion vector. The
2026-05-05 follow-up added the DBR-17.3 native cuDF path for the supported read cases,
and the 2026-05-06 cleanup removed the DBR materialized GPU fallback from the final
commit.
...
- DB-17.3 cluster uses `GpuFileSourceScanExec` for supported native DV reads of
  DV-enabled Delta tables.
- DV bitmap is loaded per file via
  `PartitionedFile.otherConstantMetadataColumnValues[FILE_ROW_INDEX_FILTER_ID_ENCODED]`
  or recovered from DBR `RowIndexFilterProvider` / `TahoeFileIndex`, converted to
  cuDF-compatible bytes, and applied by the native cuDF Parquet read path.
- DBR-17.3 DV scans fall back to CPU when either native gate is disabled.
- DBR row-index filter semantics other than `IF_CONTAINED` fall back to CPU.
- Skip-row predicate removal happens only for native GPU DV scan children; DML
  fallback plans keep DB's skip-row filter.
- DB missing-row-index-filter assertion wording is covered by a Python
  integration-test canary because Scala tests are not run for the Databricks
  platform.
- Targeted DV-read selections in
  [integration_tests/src/main/python/delta_lake_test.py](integration_tests/src/main/python/delta_lake_test.py)
  and [integration_tests/src/main/python/delta_lake_delete_test.py](integration_tests/src/main/python/delta_lake_delete_test.py)
  pass on DB-17.3 with CPU/GPU result parity and expected disabled-gate fallback.
- Builds remain green for `spark400db173`.

**Non-goals / remaining gaps**

- DBR-17.3 materialized/non-cuDF GPU DV fallback. Disabled native gates must use
  CPU fallback; OSS Delta materialized support remains separate.
- GPU DELETE/UPDATE/MERGE/OPTIMIZE on DB-17.3 (Issues 3/4/5).
- GPU DELETE/UPDATE with persistent deletion vectors. This remains CPU fallback
  on DBR and follows the OSS Delta support boundary in this repo.
...
The initial V1 path does **not** use
`com.databricks.sql.io.RowIndexFilterProvider.retrieve(conf)`. It resolves the
bitmap directly via
`StoredBitmap.create(dvDescriptor, new Path(tablePath)).load(new HadoopFileSystemDVStore(conf))`
— same shape as the OSS code. The 2026-05-05 native path uses DBR
`RowIndexFilterProvider` / `SerializedBitmap` data and converts it to the
standard portable roaring bitmap bytes cuDF expects.
...
The code block below documents the initial 2026-05-02 materialized-reader
checkpoint. As of 2026-05-05, the native path enables pushdown only when DBR
metadata-row-index mode and the RAPIDS DV predicate-pushdown config are enabled,
and the rewrite recognizes only the current DBR skip-row predicate shapes.
...
6. **`Spark400PlusDBShims.getPartitionFiles` + `FilePartitionShims.withPathPrefixIfNeeded`**
  ...
  is keyed off the absolute path. The initial V1 path did not use native predicate
  pushdown, but the 2026-05-05 native follow-up does when DBR metadata-row-index mode and
  the RAPIDS DV predicate-pushdown config are enabled. Validate the split behavior with
  `test_delta_scan_split_with_DV_enabled_with_DVs`.
...
|---|---|---|
| [`test_delta_scan_split_with_DV_enabled_with_DVs`](integration_tests/src/main/python/delta_lake_test.py#L349) | "Deletion vector scan is not supported on Databricks" | **Remove the skip on DB-17.3 only** — exactly what this PR enables. Replace the Databricks skip with `@skipif(is_databricks_runtime() and not is_databricks173_or_later(), ...)` and import `is_databricks173_or_later`; keep the existing `is_before_spark_353()` skip. Do not skip OSS Spark. Initial 2026-05-02 V1 expectation: `canPushDVPredicateDownToScan=false`, so `pushdown_dv_predicate=True` should not expect the native `GpuDeltaParquetFileFormatBase2` split behavior. 2026-05-05 native follow-up: native pushdown is enabled behind the DBR metadata-row-index and RAPIDS DV pushdown gates, and this test passed in the targeted native validation. |
