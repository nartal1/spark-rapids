...
cleanup was squashed on 2026-05-06.
...
- `DeltaSpark400DB173Provider.scala` recognizes DBR DV scan shapes, gates native
  ...
  for CPU fallback when the gates are false.
- `GpuDeltaParquetFileFormat.scala` preserves non-DV GPU Delta reads, mirrors the
  ...
  non-DV reads, and participates in disabled-gate DV fallback tagging.
...
2026-05-12/13 branch-split amend: the DBR 17.3 DV metric-scan fallback also
lives in the native-DV PR commit. It detects DELETE/UPDATE-shaped filters that
both reference the DV skip-row column and contain Databricks `IncrementMetric`,
then tags those scans for CPU fallback. That logic was removed from the
separate DELETE/UPDATE branch so DV read safety remains tracked with Issue 6.
The separate Issue 3 branch is PR-ready at
`c8a0f87b86c16ce02097ae3501c328f584bb720e` and supports non-persistent-DV
DELETE/UPDATE rewrites on GPU; DELETE/UPDATE paths that would write persistent
DVs still fall back to CPU.
...
- Only `RowIndexFilterType.IF_CONTAINED` is supported, matching OSS behavior.
...
Issue 6 was reordered ahead of Issues 3/4/5 (GPU DELETE/UPDATE/MERGE/
OPTIMIZE) because the target customers are **read-heavy**, not MERGE-heavy. DV
reads alone unlock GPU acceleration for SELECT/aggregate/join workloads on
every DV-tagged Delta table, including DV tables created by writers outside this
plugin (Databricks UI, dbt, CPU Spark jobs). As of 2026-05-13, Issue 3
DELETE/UPDATE is PR-ready separately at
`c8a0f87b86c16ce02097ae3501c328f584bb720e`; MERGE/OPTIMIZE and persistent DV
writes remain outside this Issue 6 read scope.
...
- GPU MERGE/OPTIMIZE on DB-17.3 (Issues 4/5).
- Persistent DV writes from GPU DML. Issue 3 DELETE/UPDATE keeps those paths on
  CPU while supporting non-DV row rewrites on GPU.
