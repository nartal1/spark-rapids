...
**Status:** **Finalized — implementation blueprint** (2026-04-29)
...
DB-17.3 makes deletion vectors (DVs) the default for new Delta tables. The current DB-17.3
shim explicitly blocks GPU for any DV-enabled scan in
...
| ---------------------------------- | ----------------------------------- |
| `IS_ROW_DELETED_COLUMN_NAME`       | `__delta_internal_is_row_deleted`   |
| `ROW_INDEX_COLUMN_NAME`            | `__delta_internal_row_index`        |
...
guard is **not** related to DVs — DV columns start with double-underscore-`delta`
(`__delta_internal_…`). The existing block stays as-is.
...
// Keep the existing _databricks_internal block verbatim. Verified via javap -p -v on
// DeltaParquetFileFormat$.class: DV columns are __delta_internal_is_row_deleted and
// __delta_internal_row_index (double-underscore-delta), so they do NOT collide with
// this prefix. The block guards an unrelated Databricks-internal feature.
...
- **Final critical review (2026-04-29, latest):** end-to-end review against the live
  ...
  verified during this review: `_databricks_internal` columns (`_databricks_internal_edge_cdc_id`,
  `_databricks_internal_edge_computed_column_skip_row`, `_databricks_internal_edge_cdc_id_col_`)
  are MST/CDC features, completely unrelated to DV — the existing block stays as-is.
