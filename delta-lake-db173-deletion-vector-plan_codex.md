...
2026-05-05 and the DBR-17.3 native-only cleanup squashed on 2026-05-06. The
native-only commit was amended on 2026-05-11 with unsupported row-index filter
fallback hardening and on 2026-05-14 with a native-scan guard that prevents
skip-row predicate removal in CPU-fallback DML plans plus a Python canary for
DB missing-row-index-filter assertion wording. The
source-of-truth implementation record is now
...
The final DBR-17.3 scope is **native cuDF DV reads only**. The earlier
V1/materialized DBR GPU DV reader was stripped after team review because DBR-17.3
should fall back to CPU when the native cuDF DV path is not enabled. This does
not change OSS Delta support: OSS Delta still has the materialized/non-cuDF DV
path where its modules support it.

Final DBR-17.3 gate behavior:
- GPU DV read path requires
  `spark.databricks.delta.deletionVectors.useMetadataRowIndex=true` and
...
- If either native gate is false for a DBR-17.3 DV scan, the scan is tagged for
  CPU fallback. There is no DBR-17.3 materialized Scala/GPU DV fallback in the
  final squashed commit.
- If DBR exposes a non-`IF_CONTAINED` row-index filter type, such as the
  `IF_NOT_CONTAINED` provider observed in `test_delta_merge_query`, the scan is
  also tagged for CPU fallback. The native cuDF DV path treats the bitmap as rows
  to drop and cannot implement keep-marked-row semantics directly.
- The DBR skip-row predicate is removed only when the child plan contains a
  `GpuFileSourceScanExec` using `GpuDeltaParquetFileFormatNativeDV`. DELETE/DML
  plans that fall back while writing persistent DVs keep DB's skip-row filter.

Final net files:
- Added `DeltaBitmapUtils.java`, `GpuDeltaParquetFileFormatNativeDV.scala`, and
  `RapidsDeletionVectors.scala`.
- Updated `DeltaSpark400DB173Provider.scala`, `GpuDeltaParquetFileFormat.scala`,
  `delta_lake_test.py`, and `delta_lake_delete_test.py`.
- Removed the intermediate `GpuDeltaParquetFileFormatDV.scala` and
  `RapidsRowIndexFilters.scala` from the final commit.

Final validation:
- `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh` passed.
- Targeted DBR DV read slice 1: 80 selected, 80 passed.
- Targeted DBR DV read slice 2: 120 selected, 108 passed, 12 expected
  Databricks skips.
- 2026-05-11 focused DV selection: 194 tests, 0 failures.
- 2026-05-11 broader Delta read/delete selection: 289 tests, 0 failures,
  40 skips.
- 2026-05-11 auto-compact selection: 14 tests, 0 failures.
...
The V1/materialized deletion-vector read path described here landed as an
intermediate DBR-17.3 checkpoint. The implementation added DB-17.3-local
`GpuDeltaParquetFileFormatDV`, `RapidsDeletionVectors`, and
`RapidsRowIndexFilters` files, updated `GpuDeltaParquetFileFormat` and
`DeltaSpark400DB173Provider`, and added targeted integration-test assertions
that confirmed the intermediate DBR-17.3 GPU scan path was exercised.
...
This was the initial materialized-reader delivery only. The 2026-05-05 follow-up
...
native-footer multi-row-group/count-star coverage. The 2026-05-06 cleanup then
removed the DBR materialized GPU path from the final squashed commit. Remaining
follow-up areas are CDC + DV reads, DB-17.3 CI enablement, and review of any new
DBR plan shapes.
...
- The native path is gated by DBR metadata-row-index mode and RAPIDS DV
  predicate pushdown. Other DBR-17.3 DV reads fall back to CPU.
