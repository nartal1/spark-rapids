...
Final validation: build passed, targeted DV slice 1 passed 80/80, targeted DV
slice 2 passed 108/120 with 12 expected Databricks skips.
...
Because the target customers are read-heavy and not MERGE-heavy, Issue 6 (GPU DV reads) was reordered ahead of Issues 3/4/5 (GPU DELETE/UPDATE/MERGE/OPTIMIZE) on the [delta-lake-db173-github-issues.md](delta-lake-db173-github-issues.md) plan. DV reads alone unlock GPU acceleration for SELECT/aggregate/join workloads on every DV-tagged Delta table — including DV tables created by writers outside this plugin (Databricks UI, dbt, CPU Spark jobs). As of 2026-05-13, Issue 3 DELETE/UPDATE is PR-ready separately at `c8a0f87b86c16ce02097ae3501c328f584bb720e`; this historical Issue 6 plan remains scoped to DV reads.
...
- GPU MERGE/OPTIMIZE on DB-17.3 (Issues 4/5). Issue 3 DELETE/UPDATE is
  covered by the separate PR-ready branch.
