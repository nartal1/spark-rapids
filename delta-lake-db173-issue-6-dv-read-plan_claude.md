...
**2026-05-02 result:** DB-17.3 now has the V1/materialized GPU DV read path
implemented through DB-local `GpuDeltaParquetFileFormatDV`,
`RapidsDeletionVectors`, and `RapidsRowIndexFilters`.
...
This plan originally covered only the **V1 read path**: GPU Parquet read followed by a post-read `skip_row` mask materialized from the Delta deletion vector. The 2026-05-05 follow-up adds the DBR-17.3 native cuDF path for supported read cases. DBR `SerializedBitmap` data is converted to cuDF-compatible standard portable roaring bitmap bytes through `DeltaBitmapUtils.java`.
...
- Historical 2026-05-02 non-goal now delivered locally on 2026-05-05: V2/Base2-equivalent cuDF-native DV reader path for supported DBR-17.3 reads.
