...
## Current Progress Checkpoint: May 21, 2026

Current PR branch: `delta_db173_optimize_auto_compaction`.

This branch is the smaller regular OPTIMIZE + inline auto-compaction PR
branch on top of the DBR 17.3 DELETE/UPDATE base. PR #14787 and PR #14810 are
already in the base. The May 20 baseline contained the two DBR 17.3 OPTIMIZE /
auto-compaction commits plus one follow-up fix commit:

- `37903ee00` - Add DBR 17.3 Delta OPTIMIZE support
- `704568255` - Add DBR 17.3 Delta auto compaction support
- `4bffbd2c0` - Fix DBR 17.3 Delta OPTIMIZE follow-ups

As of 2026-05-21, PR #14847 has review/CI hardening commits on top of the May 20
baseline and the current tip is `f8525abf7`:

- `15c3701e3` - addressed review comments in Scala OPTIMIZE code and fixed
  scalastyle line-length/import-order failures.
- `fcf62bc5c` - fixed license headers and OPTIMIZE/auto-compact integration
  test issues.
- `f6bd6eb34` - added scoped GPU OPTIMIZE execution validation by asserting a
  captured plan contains `GpuExecutedCommandExec`.
- `f8525abf7` - disabled auto-compaction in the manual OPTIMIZE parity test
  config to avoid interference from auto-compact defaults.

Review-comment disposition from this session:

- The GPU execution coverage comment was valid and fixed.
- The stale stats concern was valid for the retry path and is fixed by returning
  the committed transaction from `commitAndRetry`; no extra first-attempt
  snapshot refresh was added because that could report metadata from a later
  unrelated commit.
- The auto-compact interference comment was incorporated as test isolation even
  though the exact post-OPTIMIZE hook mechanism is unlikely with `isOptimize = true`.

The follow-up commit `4bffbd2c0` was added after review and validation. It:

- Adds a runtime guard in `GpuOptimizeExecutor` so writable deletion-vector
  tables cannot accidentally use the GPU OPTIMIZE executor if a future path
  bypasses planner tagging.
- Adds the DBR row-tracking preserved tag shape expected in OPTIMIZE commitInfo.
- Moves `OptimizeExecutionObserver.endInlineAutoCompact()` into the
  auto-compaction `finally` path so the inline observer is always closed.
- Matches DBR auto-compaction's default `minFileSize = maxFileSize / 2` behavior
  when `DELTA_AUTO_COMPACT_MIN_FILE_SIZE` is unset.

The older source branch `delta_db173_OPTIMIZE_AUTO_COMPACT` still has the larger
local stack that includes liquid clustering. On 2026-05-20 the unstaged follow-up
fixes on that branch were split into fixup commits and autosquashed into the
commits they belonged to. A safety branch was created before that rewrite:

- `backup/delta_db173_OPTIMIZE_AUTO_COMPACT-before-fixup-split-20260519`

Current rewritten commits on `delta_db173_OPTIMIZE_AUTO_COMPACT`:
...
- First PR: use `delta_db173_optimize_auto_compaction` at `f8525abf7`.
  This gives DBR 17.3 regular OPTIMIZE and inline auto-compaction support with
  May 21 review/CI fixes included and liquid clustering explicitly outside the
  PR scope.
...
- Issue #14599 originally includes liquid clustering. The smaller PR should use
  `Refs #14599`, not `Closes #14599`, unless the GitHub issue is split or the
  liquid clustering follow-up is intentionally folded into the same PR later.
- The documentation-only commit `afa0cd4c2` adds these planning/checkpoint docs.
  It is intentionally not part of the smaller code PR unless the docs are
  deliberately added for review.

Milestone status:

- Milestone 1 is complete in the base branch via PR #14787 and PR #14810.
- Milestone 2 is complete and present in `37903ee00` on the smaller PR branch.
- Milestone 3 is complete and present in `704568255` on the smaller PR branch.
- The May 20 follow-up fixes are complete and committed in `4bffbd2c0`.
...
- Milestone 5 is complete for the smaller PR: build and focused OPTIMIZE /
  auto-compaction tests passed after `f8525abf7`.
...
- On the smaller PR branch after `4bffbd2c0`, the requested build passed:
...
- On the smaller PR branch after `4bffbd2c0`, the focused tests passed:
...
Result observed: `19 passed`, `28 warnings`, no failures.

- `git diff --check` passed before `4bffbd2c0` was committed.

- During PR #14847 review on 2026-05-21, the requested scalastyle and license
  checks were fixed, and `git diff --check` passed after the review edits.
- The full DBR build also passed after the review edits:
  `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh` -> `BUILD SUCCESS`.

- After adding scoped GPU OPTIMIZE execution validation, the focused OPTIMIZE
  test passed:
...
Result observed: `5 passed`, `28 warnings`, `184.57s`.

- After explicitly disabling auto-compaction in `_optimize_conf`, the same
  focused OPTIMIZE test passed again: `5 passed`, `28 warnings`, `180.10s`.
...
- Row-tracking commit tags are produced through
  `RowTracking.addPreservedRowTrackingTagIfNotSet`, with an explicit
  `delta.rowTracking.preserved=false` fallback, in the DBR 17.3 optimize commit
  path. This was required for DBR commitInfo parity.
- The DBR 17.3 optimize executor now has a runtime writable-DV guard in addition
  to planner tagging, so direct/future executor entry points fail fast instead
  of writing unsupported DV tables on GPU.
...
- Inline auto-compaction now closes the `OptimizeExecutionObserver` in the
  `finally` path and uses DBR's auto-compaction min-file-size default when the
  min config is unset.
...
1. For the smaller OPTIMIZE/auto-compaction PR, use
   `delta_db173_optimize_auto_compaction` at `f8525abf7`. The code is in good
   PR shape. The untracked markdown files should remain out of the PR unless
   documentation review is desired.

Suggested PR title:

```text
Enable Delta OPTIMIZE and auto compaction for DBR 17.3 [databricks]
```
...
This PR adds the regular DBR 17.3 Delta OPTIMIZE path and inline
auto-compaction support. Liquid clustering remains out of scope for this PR and
will be handled separately.
```

The validation already run for this smaller PR was:
...
3. Before the liquid clustering PR is pushed, rerun the DBR build on its final
   branch state:
