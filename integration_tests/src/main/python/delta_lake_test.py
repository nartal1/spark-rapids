...
def _assert_db173_gpu_delta_scan_if_enabled(spark, df):
    if is_databricks173_or_later() and \
        ...
        plan = df._sc._jvm.PythonSQLUtils.explainString(df._jdf.queryExecution(), "extended")
        assert any(scan in plan for scan in ["GpuFileSourceScanExec", "GpuFileGpuScan"]), plan
    return df
...
def test_delta_deletion_vector_multithreaded_read_partitioned_table(
    ...
    assert_gpu_and_cpu_are_equal_collect(
        lambda spark: _delta_sql_with_gpu_scan_assert(
...
def do_test_scan_split(spark_tmp_path, enable_deletion_vectors, expected_num_partitions, post_setup_table_func=None, conf=None):
    ...
    def get_num_partitions(spark):
        df = _delta_sql_with_gpu_scan_assert(
            spark, "SELECT * from delta.`{}`".format(data_path))
