...
def _assert_native_optimize_gpu_write_parity(spark_tmp_path):
    ...
    try:
        ...
        gpu_write_plans = [
            ...
            if plan_callback.contains(plan, "GpuDataWritingCommandExec") and
            plan_callback.contains(plan, "GpuWriteFilesExec")
        ]
        assert gpu_write_plans, (
            "No captured native OPTIMIZE plan contains both GpuDataWritingCommandExec "
            "and GpuWriteFilesExec")
        for plan in captured_plans:
            for cpu_class in ["DataWritingCommandExec", "WriteFilesExec"]:
                assert not plan_callback.didFallBack(plan, cpu_class), \
                    f"Captured native OPTIMIZE write contains CPU {cpu_class}"
    finally:
        plan_callback.endCapture()
