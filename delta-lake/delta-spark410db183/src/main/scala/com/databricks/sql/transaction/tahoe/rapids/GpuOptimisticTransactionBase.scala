...
abstract class GpuOptimisticTransactionBase(
  ...
  override def writeFiles(
    ...
    if (isRemoveColumnMapping || cdcTypeForAllFiles.nonEmpty) {
      ...
      writeFilesAndGetQueryExecution(
        inputData,
        writeOptions,
        isOptimize,
        keepPartitionIdTag,
        additionalConstraints,
        isRemoveColumnMapping,
        cdcTypeForAllFiles,
        isEagerClustering = false,
        clusteringColumnsOpt = None)._1
    }
  }
  ...
  override protected def isOptimizeCommand(plan: LogicalPlan): Boolean = {
    ...
    leaves.size == 1 && leaves.head.collect {
      ...
      index: TahoeBatchFileIndex, _, _, _, _, _), _, _, _, _, _, _, _) =>
