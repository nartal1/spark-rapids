...
class GpuOptimisticTransaction(
  ...
  override def writeFilesAndGetQueryExecution(
    ...
    if (isCDCWritePhase) {
      super.writeFilesAndGetQueryExecution(
        ...
        additionalConstraints, isCDCWritePhase, context, trailing, clusteringColumnsOpt)
    } else {
      // DB-18.3 uses clusteringColumnsOpt only to configure page indexes for the Photon Parquet
      // writer. GPU writes do not use that writer, so retain the DB-17.3 GPU write path here.
      gpuWriteFiles(
        inputData,
        Option(writeOptions).flatMap(_.deltaOptions),
        additionalConstraints,
  ...
  override def writeFilesAndGetExecutedPlan(
    ...
    // See writeFilesAndGetQueryExecution: clusteringColumnsOpt only configures the Photon writer.
    val (fileActions, qe) =
      gpuWriteFiles(
        inputData,
        deltaOpts,
