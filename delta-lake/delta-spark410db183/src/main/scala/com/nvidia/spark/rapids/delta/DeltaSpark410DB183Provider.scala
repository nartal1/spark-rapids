...
object DeltaSpark410DB183Provider extends DatabricksDeltaProviderBase {
  ...
  override def tryPushDVPredicateDownToScan(plan: SparkPlan): SparkPlan = {
    val pushed = DB183DVPredicatePushdown.pushToScan(plan)
    DB183DVPredicatePushdown.mergeIdenticalProjects(pushed)
  }
    ...
    override def toInsertableRelation(): InsertableRelation = {
      new InsertableRelation {
        override def insert(data: DataFrame, overwrite: Boolean): Unit = {
          ...
          // The V1 write adapter does not carry DB-18.3 transaction metadata options.
  ...
  override def tagForGpu(
    ...
    tagDB183UnsupportedTableSpec(meta, cpuExec.tableSpec, cpuExec.session)
  ...
  private def tagDB183UnsupportedTableSpec(
    ...
    // DB-18.3 can enable deletion vectors during table creation from either an explicit table
...
private object DB183DVPredicatePushdown extends ShimPredicateHelper {
