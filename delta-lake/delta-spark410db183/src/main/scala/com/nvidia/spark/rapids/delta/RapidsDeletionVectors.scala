...
import com.databricks.sql.transaction.tahoe.deletionvectors.StoredBitmap
...
object RapidsDeletionVectors extends Logging {
  ...
  def loadScalaBitmap(
      conf: Configuration,
      partitionedFile: PartitionedFile,
      tablePath: String): Array[RoaringBitmap] = {
  ...
  def loadScalaBitmap(
      ...
      partitionedFile: PartitionedFile,
      tablePath: String,
      deletionVectorReadInfo: Option[RapidsDeletionVectorReadInfo]): Array[RoaringBitmap] = {
    val dv = lookupDeletionVector(partitionedFile, deletionVectorReadInfo)
    loadScalaBitmap(
      conf,
      dv.dvDescriptor,
  ...
  def loadScalaBitmap(
    ...
    if (dvDescriptorOpt.isDefined && filterTypeOpt.isDefined) {
      ...
      filterTypeOpt.get match {
        case RowIndexFilterType.IF_CONTAINED =>
          val dvStore = DeletionVectorStoreEdge.createInstance(conf, None)
          val bytes = StoredBitmap.create(dvDesc, new Path(tablePath))
            .loadSerialized(dvStore).buffer
          DeltaBitmapUtils.deserializeDeltaBitmap(bytes)
        case unexpectedFilterType => throw new IllegalStateException(
          s"Unexpected row index filter type for Deletion Vectors. " +
            s"Expected: ${RowIndexFilterType.IF_CONTAINED}; Actual: ${unexpectedFilterType}")
      }
      ...
      rowIndexFilterProviderOpt.map { provider =>
        ...
        DeltaBitmapUtils.deserializeDeltaBitmap(provider.retrieveSerialized(conf).buffer)
      }.getOrElse(Array.empty[RoaringBitmap])
    }
  }
  ...
  def countDeletedRows(
      scalaBitmap: Array[RoaringBitmap],
      rowGroupOffsets: Array[Long],
      rowGroupNumRows: Array[Int]): Long = {
    val cardinality = scalaBitmap.foldLeft(0L)(_ + _.getLongCardinality)
    RapidsDeletionVectorRowCountUtils.countMarkedRows(
      cardinality, rowGroupOffsets, rowGroupNumRows) { countDeletedRow =>
        scalaBitmap.zipWithIndex.foreach { case (bitmap, highBits) =>
          bitmap.forEach { lowBits: Int =>
            val deletedIndex = (highBits.toLong << 32) | Integer.toUnsignedLong(lowBits)
  ...
  def computeNumRowsAlive(
    ...
    RapidsDeletionVectorRowCountUtils.computeNumRowsAlive(
      totalNumRows, cardinality, chunkedBlocks) { countDeletedRow =>
        scalaBitmap.zipWithIndex.foreach { case (bitmap, highBits) =>
          bitmap.forEach { lowBits: Int =>
            val deletedIndex = (highBits.toLong << 32) | Integer.toUnsignedLong(lowBits)
            countDeletedRow(deletedIndex)
