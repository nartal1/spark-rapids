...
class GpuOptimisticTransaction(
  ...
  protected def gpuWriteFiles(
      ...
      isOptimizeOverride: Option[Boolean]): (Seq[FileAction], QueryExecution) = {
    hasWritten = true
    ...
    // GPU auto-compaction post-commit hook is not yet implemented for DB-17.3 —
    // tracked in GitHub issue #14599. Databricks' CPU auto-compact path still runs
    // when the corresponding config/table property is enabled.

    (fileActions, queryExecution)
  }

  // Legacy 3-arg entry point. DML commands (DELETE/UPDATE/MERGE/OPTIMIZE) and the
  // 7-arg `writeFiles` override in the base class funnel into this.
  override def writeFiles(
      inputData: Dataset[_],
      writeOptions: Option[DeltaOptions],
      additionalConstraints: Seq[Constraint]): Seq[FileAction] = {
    val (fileActions, _) =
      gpuWriteFiles(inputData, writeOptions, additionalConstraints, None)
    fileActions
  }

  // DB-17.3 new entry point invoked by WriteIntoDeltaEdge.write(txn).
  ...
  // DB-17.3 sibling entry point; reachable from CTAS / RTAS paths.
  override def writeFilesAndGetExecutedPlan(
    ...
    val (fileActions, qe) =
      gpuWriteFiles(inputData, deltaOpts, additionalConstraints, Some(isOptimize))
