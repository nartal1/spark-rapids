...
case class GpuMergeIntoCommand(
  ...
  private def findTouchedFiles(
    ...
    val classicSpark = spark.asInstanceOf[TrampolineConnectShims.SparkSession]
    val sourceDF = TrampolineConnectShims.createDataFrame(classicSpark, source)
        .filter(DFUDFShims.exprToColumn(incrSourceRowCountExpr))
    ...
    val targetDF = TrampolineConnectShims.createDataFrame(
      classicSpark, buildTargetPlanWithFiles(deltaTxn, dataSkippedFiles))
        .withColumn(ROW_ID_COL, monotonically_increasing_id())
        .withColumn(FILE_NAME_COL, input_file_name())
    val joinToFindTouchedFiles = sourceDF.join(targetDF,
      DFUDFShims.exprToColumn(condition), "inner")
  ...
  private def writeInsertsOnlyWhenNoMatchedClauses(
    ...
    val insertDf = sourceDF.join(targetDF, DFUDFShims.exprToColumn(condition), "leftanti")
        .select(outputCols: _*)
        .filter(DFUDFShims.exprToColumn(incrInsertedCountExpr))
  ...
  private def writeAllChanges(
    ...
    val classicSpark = spark.asInstanceOf[TrampolineConnectShims.SparkSession]
    var sourceDF = TrampolineConnectShims.createDataFrame(classicSpark, source)
        .withColumn(SOURCE_ROW_PRESENT_COL, DFUDFShims.exprToColumn(incrSourceRowCountExpr))
    var targetDF = TrampolineConnectShims.createDataFrame(classicSpark, newTarget)
    ...
    val targetRowHasNoMatch = resolveOnJoinedPlan(
      Seq(DFUDFShims.columnToExpr(col(SOURCE_ROW_PRESENT_COL).isNull))).head
    val sourceRowHasNoMatch = resolveOnJoinedPlan(
      Seq(DFUDFShims.columnToExpr(col(TARGET_ROW_PRESENT_COL).isNull))).head
  ...
  private def addMergeJoinProcessor(
    ...
    if (canReplace) {
      ...
      TrampolineConnectShims.createDataFrame(
        spark.asInstanceOf[TrampolineConnectShims.SparkSession], processedJoinPlan)
    } else {
      ...
      TrampolineConnectShims.createDataFrame(
        spark.asInstanceOf[TrampolineConnectShims.SparkSession], joinedPlan)
  ...
  private def buildTargetPlanWithFiles(
    ...
    val plan = {
      ...
      val transformed = original.transform {
        case LogicalRelation(base, _, catalogTbl, isStreaming, stream, constraints, stats) =>
          LogicalRelation(
            base,
  ...
  private def makeMetricUpdateUDF(name: String, deterministic: Boolean = false): Expression = {
    ...
    DFUDFShims.columnToExpr(u.apply())
  }
