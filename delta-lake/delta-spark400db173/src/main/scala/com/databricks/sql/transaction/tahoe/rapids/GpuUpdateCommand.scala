  ...
  private def performUpdate(
    ...
    val filesToRewrite: Seq[AddFile] = if (candidateFiles.isEmpty) {
      ...
      val data = TrampolineConnectShims.createDataFrame(
        sparkSession.asInstanceOf[TrampolineConnectShims.SparkSession], newTarget)
      ...
      val pathsToRewrite =
        withStatusCode("DELTA", UpdateCommand.FINDING_TOUCHED_FILES_MSG) {
          data.filter(DFUDFShims.exprToColumn(updateCondition))
    ...
    if (totalActions.nonEmpty) {
      ...
      val tags = DMLUtils.TaggedCommitData.empty[FileAction]
  ...
  private def rewriteFiles(
    ...
    val updatedDataFrame = UpdateCommand.withUpdatedColumns(
      ...
      targetDf
          ...
          .withColumn(UpdateCommand.CONDITION_COLUMN_NAME, DFUDFShims.exprToColumn(condition)),
