...
object DeltaSpark400DB173Provider extends DatabricksDeltaProviderBase {
  ...
  // The outer GPU atomic exec delegates staged table creation, replacement, commit, and abort to
  // DBR's native catalog. Keep feature-specific tags until each DBR table-creation semantic has
  // been validated through that path:
