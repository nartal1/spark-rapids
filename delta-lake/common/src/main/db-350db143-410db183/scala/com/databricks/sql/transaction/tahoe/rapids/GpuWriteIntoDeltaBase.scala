...
abstract class GpuWriteIntoDeltaBase(
  ...
  /** Build the commit operation metadata using the fields supported by this runtime. */
  protected def getOperation: DeltaOperations.Write = DeltaOperations.Write(
    cpuWrite.mode,
  ...
  private def hasBeenExecuted(txn: OptimisticTransaction): Boolean = {
    ...
    val executed = for {
      ...
      logInfo(s"Transaction write of version $version for application id $appId " +
          s"has already been committed in Delta table id ${DeltaLogShim.tableId(txn.deltaLog)}. " +
