...
class GpuOptimisticTransaction(deltaLog: DeltaLog,
  ...
  private def getGpuStatsColExpr(
    ...
    TrampolineConnectShims.createDataFrame(TrampolineConnectShims.getActiveSession, LocalRelation(statsDataSchema))
  ...
  override def writeFiles(
    ...
    val gpuWritePlan = TrampolineConnectShims.createDataFrame(TrampolineConnectShims.getActiveSession, RapidsDeltaWrite(normalizedQueryExecution.logical))
    ...
    SQLExecution.withNewExecutionId(queryExecution, Option("deltaTransactionalWrite")) {
      ...
      try {
        GpuDeltaFileFormatWriter.write(
          sparkSession = ClassicSparkSession.active,
          plan = physicalPlan,
