...
import org.apache.spark.sql.classic.{SparkSession => ClassicSparkSession, ExpressionUtils}
...
case class GpuUpdateCommand(
  ...
  private def performUpdate(
    ...
    val filesToRewrite: Seq[TouchedFileWithDV] = if (candidateFiles.isEmpty) {
      ...
      val touchedFilesWithDV = if (shouldWriteDeletionVectors) {
        ...
        val data = TrampolineConnectShims.createDataFrame(TrampolineConnectShims.getActiveSession, newTarget)
        ...
        val pathsToRewrite =
          withStatusCode("DELTA", UpdateCommand.FINDING_TOUCHED_FILES_MSG) {
            data.filter(ExpressionUtils.column(updateCondition))
  ...
  private def rewriteFiles(
    ...
    val (targetDf, finalOutput, finalUpdateExpressions) = UpdateCommand.preserveRowTrackingColumns(
      targetDfWithoutRowTrackingColumns = TrampolineConnectShims.createDataFrame(TrampolineConnectShims.getActiveSession, newTarget),
      snapshot = txn.snapshot,
      targetOutput = target.output,
      updateExpressions)

    val targetDfWithEvaluatedCondition = {
      val evalDf = targetDf.withColumn(UpdateCommand.CONDITION_COLUMN_NAME, ExpressionUtils.column(condition))
