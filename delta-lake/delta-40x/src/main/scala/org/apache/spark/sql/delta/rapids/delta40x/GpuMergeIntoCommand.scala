...
import org.apache.spark.sql.classic.{SparkSession => ClassicSparkSession, ColumnNodeToExpressionConverter, ExpressionUtils}
...
case class GpuMergeIntoCommand(
  ...
  private def makeMetricUpdateUDF(name: String, deterministic: Boolean = false): Expression = {
    ...
    ColumnNodeToExpressionConverter(u.apply().node)
  }
  ...
  private def findTouchedFiles(
    ...
    val sourceDF = getMergeSource.df
      .withColumn(SOURCE_ROW_PRESENT_COL, ExpressionUtils.column(incrSourceRowCountExpr))
    ...
    val joinToFindTouchedFiles =
      sourceDF.join(targetDF, ExpressionUtils.column(condition), joinType)
    ...
    val collectTouchedFiles = joinToFindTouchedFiles
      .select(col(ROW_ID_COL),
        recordTouchedFileName(col(FILE_NAME_COL), ExpressionUtils.column(matchedPredicate)).as("one"))
    ...
    val (multipleMatchCount, multipleMatchSum) = matchedRowCounts
      .filter("count > 1")
      .select(coalesce(count(lit(1)), lit(0)), coalesce(sum("count"), lit(0)))
      .as[(Long, Long)]
      .collect()
      .head
