...
ColumnNodeToExpressionConverter, ExpressionUtils}
...
case class GpuMergeIntoCommand(
  ...
  private def makeMetricUpdateUDF(name: String, deterministic: Boolean = false): Expression = {
    ...
    org.apache.spark.sql.nvidia.DFUDFShims.columnToExpr(u())
  }
  ...
  private def findTouchedFiles(
    ...
    val sourceDF = getMergeSource.df
      .withColumn(SOURCE_ROW_PRESENT_COL, org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(incrSourceRowCountExpr))
    ...
    val joinToFindTouchedFiles =
      sourceDF.join(targetDF, org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(condition), joinType)
    ...
    val collectTouchedFiles = joinToFindTouchedFiles
      .select(col(ROW_ID_COL),
        recordTouchedFileName(col(FILE_NAME_COL), org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(matchedPredicate)).as("one"))
