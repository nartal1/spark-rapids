...
import org.apache.spark.sql.functions.{expr, input_file_name, lit}
          ...
          if (shouldWriteDVs) {
            ...
            if (filesToRewrite.isEmpty) {
              ...
              val targetDF = RowTracking.preserveRowTrackingColumns(
                dfWithoutRowTrackingColumns = TrampolineConnectShims.createDataFrame(TrampolineConnectShims.getActiveSession, newTarget),
      ...
      val dfToWrite = if (shouldWriteCdc) {
        ...
        baseData
            ...
            .withColumn(
              ...
              lit(null).cast("integer") // TODO: Fix Column constructor for If expression
            )
      } else {
        baseData
            .filter(numTouchedRowsUdf())
