          ...
          if (shouldWriteDVs) {
            ...
            val filesToRewrite =
              withStatusCode("DELTA", FINDING_TOUCHED_FILES_MSG) {
                ...
                data.filter(org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(cond))
            ...
            if (filesToRewrite.isEmpty) {
              ...
              val targetDF = RowTracking.preserveRowTrackingColumns(
                dfWithoutRowTrackingColumns = org.apache.spark.sql.rapids.shims.TrampolineConnectShims
                  .createDataFrame(sparkSession, newTarget),
      ...
      val dfToWrite = if (shouldWriteCdc) {
        ...
        baseData
            ...
            .withColumn(
              ...
              org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(If(filterCondition, CDC_TYPE_NOT_CDC, CDC_TYPE_DELETE))
            )
      } else {
        baseData
            .filter(numTouchedRowsUdf())
            .filter(org.apache.spark.sql.nvidia.DFUDFShims.exprToColumn(filterCondition))
      }
