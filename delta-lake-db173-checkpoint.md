...
**Issue 5 OPTIMIZE/auto-compaction PR review:** 2026-05-21
...
areas are MERGE, liquid clustered OPTIMIZE / `OPTIMIZE FULL`, CDC + DV reads,
non-`IF_CONTAINED` native DV filter semantics, and CI enablement. The smaller
regular OPTIMIZE + inline auto-compaction work is in PR #14847; the current
review-fix tip is `f8525abf7`.
The 2026-05-14 hardening keeps DBR DML/DELETE fallback plans from losing
...
**Current PR-prep branch:** `delta_db173_optimize_auto_compaction` at `f8525abf7`
**Historical Issue 6 branch:** `delta_db173_dv_native_ONLY_support`

---

## Issue 5 PR #14847 Review/CI Update (2026-05-21)

PR #14847 (`delta_db173_optimize_auto_compaction`) now carries the smaller DBR
17.3 regular OPTIMIZE + inline auto-compaction scope. Current tip:
`f8525abf7`.

Commits added during review/CI hardening after the May 20 baseline:

- `15c3701e3` - addressed review comments in Scala OPTIMIZE code and fixed the
  scalastyle line-length/import-order failures.
- `fcf62bc5c` - fixed license headers and restored OPTIMIZE integration-test log
  parity behavior.
- `f6bd6eb34` - added scoped plan-capture validation that GPU OPTIMIZE executes
  through `GpuExecutedCommandExec`.
- `f8525abf7` - explicitly disabled auto-compaction in the manual OPTIMIZE test
  config to keep OPTIMIZE parity tests isolated from auto-compact defaults.

Specific review outcomes:

- `FileSizeStatsWithHistogram.create(...).get` is now guarded with `getOrElse`
  and a diagnostic `IllegalStateException`.
- `commitAndRetry` now returns the transaction that actually committed, and
  returned OPTIMIZE stats use that committed transaction's metadata.
- The remaining reviewer claim about first-attempt metadata staleness was not
  incorporated; refreshing after a successful commit could report metadata from
  a later unrelated commit instead of the OPTIMIZE transaction.
- The GPU execution coverage concern was valid and fixed in
  `delta_lake_optimize_table_test.py` with `ExecutionPlanCaptureCallback` scoped
  only around the GPU `OPTIMIZE` SQL call.
- The auto-compaction test-interference concern was partially valid as a test
  isolation point. The exact OPTIMIZE-after-hook mechanism is unlikely because
  DBR 17.3 suppresses auto-compact registration when `isOptimize = true`, but
  the config guard is harmless and now present.

Validation completed on DBR 17.3 in this review session:
...
- Focused OPTIMIZE run after scoped GPU execution assertion -> `5 passed`,
  `28 warnings`, `184.57s`.
- Focused OPTIMIZE run after explicit auto-compact disable -> `5 passed`,
  `28 warnings`, `180.10s`.
...
## What's Next (Follow-up Issues — updated 2026-05-20 after OPTIMIZE/autocompaction PR prep)
...
| Issue 3 | Enable GPU DELETE + UPDATE | 7 | **Done** in PR #14810 / base `2ba5bc25e` |
| Issue 4 | Enable GPU MERGE INTO | 7 | Pending — depends on 2 |
| Issue 5 | Enable GPU OPTIMIZE + auto-compaction | 7 | **PR #14847 in review** — regular OPTIMIZE/autocompaction branch `delta_db173_optimize_auto_compaction` is at `f8525abf7` after May 21 review/CI hardening; liquid clustering follow-up remains in `0141241f0` + `a05033865` |
...
### Issue 5 local branch update (2026-05-20)

`delta_db173_OPTIMIZE_AUTO_COMPACT` was autosquashed into a clean four-commit
stack: `eda0aa172` (OPTIMIZE), `9ae539a3b` (auto-compaction), `0141241f0`
(liquid clustered OPTIMIZE), and `a05033865` (liquid write-path completion).
For the smaller PR, the active branch is now `delta_db173_optimize_auto_compaction`
on top of `origin/main` / `2ba5bc25e`; it contains `37903ee00` (OPTIMIZE),
`704568255` (auto-compaction), and `4bffbd2c0` (follow-up fixes). The follow-up
fix commit adds the runtime writable-DV guard, DBR row-tracking commitInfo tag
parity, exception-safe inline auto-compaction observer cleanup, and DBR's
auto-compaction min-file-size default. Build passed and the focused
`delta_lake_optimize_table_test.py` + `delta_lake_auto_compact_test.py` run
passed with `19 passed`. Keep liquid clustering for a separate PR. The
documentation-only commit `afa0cd4c2` maps to these untracked markdown files and
is intentionally outside the smaller code PR unless docs are explicitly added.
