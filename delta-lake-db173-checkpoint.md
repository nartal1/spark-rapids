...
## What's Next (Follow-up Issues — updated 2026-05-20 after OPTIMIZE/autocompaction split)
...
| Issue 5 | Enable GPU OPTIMIZE + auto-compaction | 7 | **Implemented locally** — split PR recommended: regular OPTIMIZE/autocompaction through `9ae539a3b`, liquid clustering follow-up in `0141241f0` + `a05033865` |
...
(liquid clustered OPTIMIZE), and `a05033865` (liquid write-path completion). The
recommended review split is to open the smaller OPTIMIZE/auto-compaction PR at
`9ae539a3b` and keep the two liquid clustering commits for a follow-up PR. The
rewrite preserved the tracked `delta-lake/` and `integration_tests/` content from
the pre-rebase fixup-chain tip and left only these documentation files untracked.
