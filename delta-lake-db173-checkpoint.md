...
**DELETE/UPDATE split branch:** 2026-05-12; **DELETE/UPDATE PR-ready review:** 2026-05-13
**Status:** Issue 1 (scaffolding) + Issue 2 (GPU writes) + Issue 6
(GPU DV reads, final DBR-17.3 native cuDF-only path) + Issue 8 + Issue 10 are
complete. Current `delta_db173_DELETE_UPDATE_support` work adds Issue 3
DELETE/UPDATE support as a separate follow-on branch, with native DV read code
removed from this branch and left to the DV PR. The final reviewed Issue 3 HEAD is
`c8a0f87b86c16ce02097ae3501c328f584bb720e` (`Support Update+DELETE for
DB-17.3`). `delta_lake_write_test.py`, targeted DV reads, and targeted
DELETE/UPDATE tests are green on DB-17.3 for the validated local runs below.
Simple Delta writes and non-persistent-DV DELETE/UPDATE rewrites run on GPU;
DELETE/UPDATE paths that would write persistent deletion vectors still fall back
to CPU. DV-bearing DBR-17.3 Delta
reads run on GPU only through the separate native cuDF DV branch/PR; without
that branch they remain CPU fallback in this DELETE/UPDATE branch. MERGE,
OPTIMIZE, CDC + DV reads, and CI enablement remain follow-up areas. A follow-up
refactor moved DB-14.3/DB-17.3 transaction, catalog, data-source, and shim code into
`delta-lake/common/src/main/db-350db143-400db173/scala`; the DB-17.3 module now
keeps only version-specific adapters, local command shims, and branch-specific
read/DML files.
**Build verified with:** `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh`
**Branch:** `delta_db173_DELETE_UPDATE_support`
...
- Build: `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh`
  passed after the native-only cleanup.
...
## DELETE/UPDATE Branch Split, Review, and Validation (2026-05-12/13)

Current branch:

```text
delta_db173_DELETE_UPDATE_support
c8a0f87b86c16ce02097ae3501c328f584bb720e Support Update+DELETE for DB-17.3
```

The branch was cleaned so it no longer carries the native-DV read commit
`25d3e04c9`. The DV metric-scan fallback hunks moved to the separate native-DV
branch, where they are squashed into:

```text
844678a6e [databricks] Add native-only Delta DV reads for DBR 17.3
```

Final DELETE/UPDATE branch scope:
- Adds shared DB-14.3/DB-17.3 `GpuDeleteCommandBase.scala` and
  `GpuUpdateCommandBase.scala` under `common/src/main/db-350db143-400db173/scala`.
- Turns DB-17.3 `GpuDeleteCommand.scala` and `GpuUpdateCommand.scala` from
  compile-only stubs into thin adapters over those shared bases.
- Updates `DeleteCommandMetaShim.scala` and `UpdateCommandMetaShim.scala` so
  DELETE/UPDATE can run on GPU except when the command would write persistent
  deletion vectors.
- Preserves Delta row-tracking columns during DELETE and UPDATE rewrites.
- Reports metadata-derived affected row counts for whole-table and metadata-only
  DELETE when `spark.databricks.delta.dmlMetricsFromMetadata.enabled` is enabled.
- Keeps plan-time CPU fallback for persistent deletion-vector write paths, with
  runtime guards as a defensive backstop.
- Adds DBR-17.3 regression coverage for row tracking, whole-table DELETE metadata
  metrics, metadata-only DELETE metrics, and hardened latest-Delta-log comparison
  for row-tracking DML tests.
- Keeps native DV read files, `RapidsDeletionVectors`, `DeltaBitmapUtils`, and
  the DV metric-scan fallback out of this branch. Those belong to the DV PR.

Validation on DBR 17.3:
- Build passed:
  `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh`.
- Focused DBR-17.3 DELETE/UPDATE regressions passed:
  `4 tests, 0 failures, 0 errors, 0 skipped` with
  `-k "test_delta_delete_entire_table_reports_row_count_db173 or
  test_delta_delete_metadata_only_reports_row_count_db173 or
  test_delta_delete_preserves_row_tracking_db173 or
  test_delta_update_preserves_row_tracking_db173"`.
- Focused persistent-DV write fallback coverage passed:
  `3 tests, 0 failures, 0 errors, 0 skipped` with
  `-k "test_delta_deletion_vector_fallback or
  test_delta_update_fallback_with_deletion_vectors"`.
- Earlier broad DELETE/UPDATE file validation: full `delta_lake_update_test.py`
  passed; full `delta_lake_delete_test.py` had failures only in native DV read cases
  (`test_delta_deletion_vector_read*`) that are intentionally owned by the separate
  DV PR.
- Databricks printed a noisy shutdown warning
  `Could not create logging file: Permission denied`, but the narrowed pytest
  run exited successfully and its XML report was clean.
...
### A. DB-17.3 Module: `delta-lake/delta-spark400db173/` (post-refactor + Issue 3 branch, 2026-05-12)
...
delta-lake/delta-spark400db173/
  ...
  src/main/scala/
    com/databricks/sql/transaction/tahoe/rapids/
      ...
      GpuDeleteCommand.scala             -- DB-17.3 adapter over shared DELETE base
      ...
      GpuUpdateCommand.scala             -- DB-17.3 adapter over shared UPDATE base
    ...
    com/nvidia/spark/rapids/delta/shims/
      DeleteCommandMetaShim.scala        -- GPU DELETE unless persistent DV writes would be used
      ...
      UpdateCommandMetaShim.scala        -- GPU UPDATE unless persistent DV writes would be used
```
...
delta-lake/common/src/main/db-350db143-400db173/scala/
  com/databricks/sql/transaction/tahoe/rapids/
    ...
    GpuDeltaCatalog.scala
    GpuDeltaCatalogCommon.scala
    GpuDeleteCommandBase.scala
    GpuOptimisticTransactionBaseCommon.scala
    GpuOptimisticTransactionWriteBase.scala
    GpuUpdateCommandBase.scala
    GpuWriteIntoDeltaBase.scala
  com/nvidia/spark/rapids/delta/
    GpuDeltaDataSource.scala
  com/nvidia/spark/rapids/delta/shims/
...
## DML Command Shims: Post-slim-down Baseline and DELETE/UPDATE Follow-up
...
Current `delta_db173_DELETE_UPDATE_support` supersedes that baseline for
DELETE/UPDATE only. Final reviewed HEAD
`c8a0f87b86c16ce02097ae3501c328f584bb720e` has `GpuDeleteCommand` and
`GpuUpdateCommand` executing through the shared `GpuDeleteCommandBase` /
`GpuUpdateCommandBase` implementations, while `MergeIntoCommandMetaShim` and
OPTIMIZE-related paths remain CPU fallback. The DELETE/UPDATE shims still tag CPU
fallback when persistent deletion-vector writes would be used.
...
## What's Next (Follow-up Issues — updated 2026-05-06 after native DV cleanup)
...
| Issue 3 | Enable GPU DELETE + UPDATE | 7 | **PR-ready locally** on `delta_db173_DELETE_UPDATE_support` at `c8a0f87b86c16ce02097ae3501c328f584bb720e` (2026-05-13); focused DELETE/UPDATE regressions and persistent-DV write fallback tests pass |
| Issue 4 | Enable GPU MERGE INTO | 7 | Pending — depends on 2 |
| Issue 5 | Enable GPU OPTIMIZE + auto-compaction | 7 | Pending — depends on 2 |
| Issue 6 | GPU Deletion Vector reads — rewrite `GpuDeltaParquetFileFormat` | 7 | **Done** (2026-05-06) — DB-17.3 native cuDF-only DV read path |
