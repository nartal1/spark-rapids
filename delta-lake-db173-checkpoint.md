...
(GPU DV reads) + Issue 8 + Issue 10 complete. `delta_lake_write_test.py`,
`delta_lake_test.py`, and `delta_lake_delete_test.py` are green on DB-17.3
for the validated local runs below. Simple Delta writes run on GPU end-to-end
with data + log parity, and DV-bearing Delta reads now use the DB-17.3
V1/materialized GPU read path. DML (DELETE/UPDATE/MERGE) and OPTIMIZE remain
follow-up CPU-fallback areas.
...
`canPushDVPredicateDownToScan=false`, and prunes Delta metadata columns for the
materialized DV plan shape.
...
Remaining after Issue 6: native cuDF DV reader / Base2-equivalent path, DV
predicate pushdown, split/native optimized DV scanning, true DV-aware
coalescing, CDC + DV reads, and DB-17.3 CI Delta enablement (Issue 7).
