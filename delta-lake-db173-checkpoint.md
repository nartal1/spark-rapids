...
**Native DV follow-up:** 2026-05-05
**Status:** Issue 1 (scaffolding) + Issue 2 (GPU writes) + Issue 6
(GPU DV reads, including the 2026-05-05 native cuDF follow-up) + Issue 8 +
Issue 10 complete. `delta_lake_write_test.py`, `delta_lake_test.py`, and
`delta_lake_delete_test.py` are green on DB-17.3 for the validated local runs
below. Simple Delta writes run on GPU end-to-end with data + log parity, and
DV-bearing Delta reads now support both the DB-17.3 V1/materialized GPU read
path and the DB-17.3 native cuDF DV path. DML (DELETE/UPDATE/MERGE), OPTIMIZE,
CDC + DV reads, and CI enablement remain follow-up CPU-fallback areas.
...
## Issue 6 DV Read Delivery (2026-05-02)

Issue 6 landed the DB-17.3-local V1/materialized deletion-vector read path. The
GPU file format now mirrors the DB-17.3 `DeltaParquetFileFormat` constructor
state, allows the DB edge skip-row column, loads per-file row-index filters from
Databricks DV metadata, and applies the skip mask after the Parquet read. The
provider now recognizes the DB-17.3 GPU Delta format, keeps
`canPushDVPredicateDownToScan=false` for the initial materialized path, and
prunes Delta metadata columns for the materialized DV plan shape.

Files added for Issue 6:
...
## Native cuDF DV Follow-up (2026-05-05)

The native DV follow-up is implemented locally and supersedes the "Remaining
after Issue 6" native-reader items above. The 2026-05-02 checkpoint should now
be read as the initial materialized-reader delivery; the 2026-05-05 checkpoint
adds the OSS Delta-4.0 parity pieces for normal DB-17.3 DV reads.

Files added or updated in this follow-up:
- `DeltaSpark400DB173Provider.scala`
- `RapidsDeletionVectors.scala`
...
Validation completed after the native follow-up:
- Build passed:
...
- The native path requires DBR metadata-row-index mode plus the RAPIDS DV
  predicate-pushdown config. Otherwise the reader falls back to the Scala/GPU
  materialized DV path.
...
### A. New Module: `delta-lake/delta-spark400db173/` (post-slim-down, 2026-04-24)
...
delta-lake/delta-spark400db173/
  ...
  src/main/scala/
    com/databricks/sql/transaction/tahoe/rapids/
      GpuCreateDeltaTableCommand.scala   -- copied from DB-14.3, fixed Dataset.ofRows (CTAS/RTAS write path)
      GpuDeleteCommand.scala             -- STUB (~30 LOC, throws on run) — full impl in issue #14597
      GpuDeltaCatalog.scala              -- copied from DB-14.3, unchanged (V2 catalog for CTAS/RTAS)
      ...
      GpuOptimisticTransaction.scala     -- Spark 4.0 fixes + Issue 2 writeFilesAndGetExecutedPlan/QueryExecution overrides (auto-compact hook dropped)
      GpuUpdateCommand.scala             -- STUB (~35 LOC, throws on run) — full impl in issue #14597
      GpuWriteIntoDelta.scala            -- copied from DB-14.3 + Issue 2 NoRowsCopiedTag stamp
...
| Issue 6 | GPU Deletion Vector reads — rewrite `GpuDeltaParquetFileFormat` | 7 | **Done** (2026-05-02) — DB-17.3 V1/materialized DV read path |
