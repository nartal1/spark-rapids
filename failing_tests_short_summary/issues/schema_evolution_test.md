## [DBR 17.3] Integration test failures in schema_evolution_test.py

**Describe the bug**
2 integration test(s) failed in `schema_evolution_test.py` when running on Databricks Runtime 17.3 (spark400db173 shim).

```
======================= 2 failed, 28 warnings in 59.86s ========================
```
...
FAILED ../../../../integration_tests/src/main/python/schema_evolution_test.py::test_column_add_after_partition[parquet][DATAGEN_SEED=1771008949, TZ=UTC, IGNORE_ORDER({'local': True})] - py4j.protocol.Py4JJavaError: An error occurred while calling o369.saveAsTable.
...
3. Run failing tests:
   ```
   TESTS=schema_evolution_test.py WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/test.sh
   ```
