## [DBR 17.3] Integration test failures in parquet_test.py

**Describe the bug**
26 integration test(s) failed in `parquet_test.py` when running on Databricks Runtime 17.3 (spark400db173 shim).

```
= 26 failed, 3441 passed, 53 skipped, 196 xpassed, 1207 warnings in 5944.12s (1:39:04) =
```
...
```
FAILED ../../../../integration_tests/src/main/python/parquet_test.py::test_buckets[parquet-reader_confs4][DATAGEN_SEED=1771110350, TZ=UTC, INJECT_OOM, IGNORE_ORDER, ALLOW_NON_GPU(DataWritingCommandExec,ExecutedCommandExec,WriteFilesExec)] - py4j.protocol.Py4JJavaError: An error occurred while calling o755875.saveAsTable.
...
3. Run failing tests:
   ```
   TESTS=parquet_test.py WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/test.sh
   ```
