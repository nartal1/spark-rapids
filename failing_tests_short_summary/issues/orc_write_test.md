## [DBR 17.3] Integration test failures in orc_write_test.py

**Describe the bug**
29 integration test(s) failed in `orc_write_test.py` when running on Databricks Runtime 17.3 (spark400db173 shim).

```
= 29 failed, 87 passed, 22 xfailed, 2 xpassed, 124 warnings in 405.16s (0:06:45) =
```
...
FAILED ../../../../integration_tests/src/main/python/orc_write_test.py::test_write_save_table_orc[native-[Byte, Short, Integer, Long, Float, Double, String, Date, Timestamp, Decimal(7,3), Decimal(12,2), Decimal(20,2)]][DATAGEN_SEED=1771002292, TZ=UTC, INJECT_OOM] - py4j.protocol.Py4JJavaError: An error occurred while calling o340.saveAsTable.
...
3. Run failing tests:
   ```
   TESTS=orc_write_test.py WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/test.sh
   ```
