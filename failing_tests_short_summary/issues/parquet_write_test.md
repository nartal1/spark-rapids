## [DBR 17.3] Integration test failures in parquet_write_test.py

**Describe the bug**
123 integration test(s) failed in `parquet_write_test.py` when running on Databricks Runtime 17.3 (spark400db173 shim).

```
===== 123 failed, 260 passed, 5 skipped, 358 warnings in 797.97s (0:13:17) =====
```
...
FAILED ../../../../integration_tests/src/main/python/parquet_write_test.py::test_write_save_table[[Binary]][DATAGEN_SEED=1771002881, TZ=UTC, INJECT_OOM] - py4j.protocol.Py4JJavaError: An error occurred while calling o22705.saveAsTable.
...
3. Run failing tests:
   ```
   TESTS=parquet_write_test.py WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/test.sh
   ```
