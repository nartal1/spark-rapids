## [DBR 17.3] Integration test failures in explain_test.py

**Describe the bug**
4 integration test(s) failed in `explain_test.py` when running on Databricks Runtime 17.3 (spark400db173 shim).

```
================== 4 failed, 2 passed, 29 warnings in 42.21s ===================
```
...
FAILED ../../../../integration_tests/src/main/python/explain_test.py::test_explain_bucketd_scan[DATAGEN_SEED=1771008896, TZ=UTC, ALLOW_NON_GPU(ANY)] - py4j.protocol.Py4JJavaError: An error occurred while calling o512.saveAsTable.
...
3. Run failing tests:
   ```
   TESTS=explain_test.py WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/test.sh
   ```
