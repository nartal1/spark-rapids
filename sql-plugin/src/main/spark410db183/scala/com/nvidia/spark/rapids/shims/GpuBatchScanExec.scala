...
case class GpuBatchScanExec(
  ...
  override def hashCode(): Int =
    Objects.hashCode(batch, runtimeFilters, keyGroupedPartitioning, dataFilters)
  ...
  @transient protected lazy val filteredPartitions: Seq[Option[InputPartition]] = {
    ...
    if (dataSourceFilters.nonEmpty) {
      // The cast is safe because runtime filters are only assigned when the scan can be filtered.
      val filterableScan = scan.asInstanceOf[SupportsRuntimeV2Filtering]
      filterableScan.filter(dataSourceFilters.toArray)

      // Call toBatch again to obtain the filtered partitions.
  ...
  override def doCanonicalize(): GpuBatchScanExec = {
    this.copy(
      ...
      dataFilters = QueryPlan.normalizePredicates(dataFilters, output))
  }

  override def simpleString(maxFields: Int): String = {
    val truncatedOutputString = truncatedString(output, "[", ", ", "]", maxFields)
    val runtimeFiltersString = s"RuntimeFilters: ${runtimeFilters.mkString("[", ",", "]")}"
    val dataFiltersString = if (dataFilters.nonEmpty) {
      s" DataFilters: ${dataFilters.mkString("[", ",", "]")}"
    } else {
      ""
    }
    val result = s"$nodeName$truncatedOutputString ${scan.description()} " +
      s"$runtimeFiltersString$dataFiltersString"
    redact(result)
  }
