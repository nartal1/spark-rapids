...
object WritePythonUDFUtils {
  def writeUDFs(
    ...
    if (argNames.isDefined) {
      ...
      PythonUDFRunner.writeUDFs(dataOut, funcs, argMetas, udfLogMaxEntries, udfLogLevel)
    } else {
      PythonUDFRunner.writeUDFs(dataOut, funcs, argOffsets, udfLogMaxEntries, udfLogLevel)
    }
  }
}
