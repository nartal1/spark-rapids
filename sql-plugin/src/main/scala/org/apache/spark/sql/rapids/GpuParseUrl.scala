...
import com.nvidia.spark.rapids.jni.{ParseURI, ExceptionWithRowIndex}
...
case class GpuParseUrl(children: Seq[Expression], failOnError: Boolean) 
  ...
  private def handleParseException(rowException: ExceptionWithRowIndex, 
                                  part: String): Nothing = {
    val errorRowIndex = rowException.getRowIndex
    throw new SparkException(
      s"Invalid URL in parse_url($part) at row $errorRowIndex")
  }
            ...
            case partScalar: GpuScalar =>
              try {
                ...
                case rowException: ExceptionWithRowIndex if failOnError =>
                  val part = partScalar.getValue.asInstanceOf[UTF8String].toString
                  handleParseException(rowException, part)
              }
            case _ =>
              ...
              case (urlCv: GpuColumnVector, partScalar: GpuScalar, keyScalar: GpuScalar) =>
                try {
                  ...
                  case rowException: ExceptionWithRowIndex if failOnError =>
                    ...
                    System.out.println(s"urlValue: $urlValue")
                    handleParseException(rowException, s"$part, url='$urlValue'")
                }
