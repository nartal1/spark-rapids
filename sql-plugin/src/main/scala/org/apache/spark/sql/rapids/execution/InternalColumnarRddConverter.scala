...
//import org.apache.spark.sql.rapids.shims.TrampolineUtilsShims.DataFrameShims
...
object InternalColumnarRddConverter extends Logging {
  ...
  def convert(df: DataFrame): RDD[Table] = {
    ...
    val b = batch.getOrElse({
      ...
      val conf = new RapidsConf(df.sparkSession.sqlContext.sparkSession.sessionState.conf)
