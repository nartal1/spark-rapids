...
import org.apache.spark.sql.rapids.shims.StreamingShims
import org.apache.spark.sql.rapids.shims.StreamingShims.{FileStreamSinkType, MetadataLogFileIndexType}
  ...
  def resolveRelation(checkFilesExist: Boolean = true): BaseRelation = {
    val relation = (originalProvidingInstance(), userSpecifiedSchema) match {
      ...
      case (format: FileFormat, _)
          if StreamingShims.FileStreamSink.hasMetadata(
            caseInsensitiveOptions.get("path").toSeq ++ paths,
            newHadoopConfiguration(),
            sparkSession.sessionState.conf) =>
        val basePath = new Path((caseInsensitiveOptions.get("path").toSeq ++ paths).head)
        val fileCatalog = new MetadataLogFileIndexType(sparkSession, basePath,
