 ...
 * Matches Spark 4.1's SQLPartitioningAwareUnionRDD behavior (SPARK-52921).
 */
private[shims] class GpuPartitionerAwareUnionRDD(
  ...
  override def compute(partition: Partition, context: TaskContext): Iterator[ColumnarBatch] = {
    val unionPartition = partition.asInstanceOf[GpuPartitionerAwareUnionPartition]
    unionPartition.parentPartitions.iterator.zipWithIndex.flatMap { case (parentPartition, idx) =>
      rdds(idx).iterator(parentPartition, context).map { batch =>
        numOutputBatches += 1
