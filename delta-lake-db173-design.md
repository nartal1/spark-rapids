...
**Status:** Issue 1 + Issue 2 + Issue 6 + Issue 8 + Issue 10 delivered; Issues
3/4/5/7/9 pending. `delta_lake_write_test.py`, `delta_lake_test.py`, and
...
- `DeltaSpark400DB173Provider.scala` wires DB-17.3 DV scan detection,
  ...
  `canPushDVPredicateDownToScan=false` for this V1 path.
...
Remaining design work stays deferred: native cuDF DV reader support, DV predicate
pushdown, split/native optimized DV scanning, true DV-aware coalescing, CDC + DV
reads, and Issue 7 CI enablement.
...
- Similarly, the V1 DV utility code (`RapidsRowIndexFilters`,
  `RapidsDeletionVectors`) is ported with Databricks namespace imports.
