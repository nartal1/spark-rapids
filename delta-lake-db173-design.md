...
**Status:** Issue 1 + Issue 2 + Issue 6, including the native cuDF DV follow-up,
as well as Issue 8 + Issue 10 delivered; Issues 3/4/5/7/9 pending. `delta_lake_write_test.py`,
`delta_lake_test.py`, and `delta_lake_delete_test.py` are green on DB-17.3 for
the validated local runs.
...
the 2026-05-05 follow-up in §0.H. CDC + DV reads and Issue 7 CI enablement
remain deferred.
...
- Native DV remains gated by metadata-row-index mode and the RAPIDS DV pushdown
  config. Other DV reads use the materialized Scala/GPU path.
