...
**DB14.3/DB17.3 dedupe refactor:** 2026-05-06 — see §0.G below.
**Status:** Issue 1 + Issue 2 + Issue 8 + Issue 10 delivered; Issues 3/4/5/6/7/9
pending. `delta_lake_write_test.py` and `delta_lake_test.py` are green on DB-17.3.
...
**Implementation summary** (updated after the 2026-05-06 shared-root refactor):

1. Shared `GpuOptimisticTransactionWriteBase.scala` under
   `common/src/main/db-350db143-400db173/scala` owns the common DB-14.3/DB-17.3 GPU
   write pipeline (`gpuWriteFiles`). DB-17.3 `GpuOptimisticTransaction.scala` adds
   the `writeFilesAndGetExecutedPlan` and `writeFilesAndGetQueryExecution` entry points
   and routes the actual write through DB-17.3's `GpuDeltaFileFormatWriter.write`.
2. `GpuDeltaFileFormatWriter.scala` — overrides `createTaskAttemptContext` to wrap
...
3. Shared `GpuWriteIntoDeltaBase.scala` owns the transaction/idempotency flow. DB-17.3
   `GpuWriteIntoDelta.scala` stamps `NoRowsCopiedTag = true` on non-`replaceWhere`
   commits via `DMLUtils.TaggedCommitData(actions).withTag(...)` → 3-arg
   `txn.commit(actions, op, stringTags)`. Needed for Delta-log parity with CPU —
...
4. Shared `GpuDeltaDataSource.scala`, `GpuCreateDeltaTableCommandBase.scala`,
   `GpuDeltaCatalogCommon.scala`, and `GpuOptimisticTransactionBaseCommon.scala` remove
   the remaining DB-14.3/DB-17.3 duplication. Version adapters retain constructor and
...
The implementation now follows the earlier `delta-33x-40x` pattern more closely:
common behavior lives in a shared source root and each supported runtime keeps only the
API differences as thin adapters.

The shared source root is intentionally narrow:

```text
delta-lake/common/src/main/db-350db143-400db173/scala/
```

It is injected only into:

- `delta-spark350db143`
- `delta-spark400db173`

It is **not** part of `common/src/main/databricks/scala`, so Databricks 12.2, 13.3,
and other runtimes do not compile these files.

Shared base/common classes:
...
- shared `com.nvidia.spark.rapids.delta.GpuDeltaDataSource`
- shared shims: `DeltaLogShim`, `InvariantViolationExceptionShim`, `MetadataShims`,
  `ShimDeltaUDF`

The DB-14.3 and DB-17.3 module files now mostly express version-specific API seams:

- DB-14.3 owns the older `DeltaInvariantCheckerExec(cpuPlan, constraints)`, the
  old `LogicalRelation` pattern shape, the 2-arg `writeFiles` delegator, and
  `GpuDoAutoCompaction`.
- DB-17.3 owns the Spark-4 / Delta-17.3 API differences: 3-arg
  `DeltaInvariantCheckerExec`, 7-field `LogicalRelation`, the
  `TransactionalWriteOptions` write methods, `writeFilesAndGetExecutedPlan`,
  `writeFilesAndGetQueryExecution`, `RuntimeReplaceable` stats-expression handling,
  `GpuDeltaFileFormatWriter`, and `NoRowsCopiedTag`.

This refactor changes the design interpretation of §4.1.3 below: the listed DB-17.3
command classes are no longer full copied implementations. Create-table, catalog,
transaction, data-source, and write-into-delta logic is mostly shared between DB-14.3
and DB-17.3, with adapters preserving the runtime-specific contracts.

Final DB-17.3-specific Scala size after the refactor is about 983 raw LOC, or 602
non-comment/nonblank LOC. GitHub PR additions remain larger because the shared
`db-350db143-400db173` files are new relative to `main`.
...
Before this work, the `release400db173` Maven profile used `delta-lake/delta-stub`,
which provides a no-op `NoDeltaProvider`. The PR replaces that with
`delta-lake/delta-spark400db173`; simple Delta writes run on GPU, while CTAS/RTAS,
DELETE, UPDATE, MERGE, OPTIMIZE, and DV reads still fall back to CPU.
...
├── delta-spark350db143/             # Databricks 14.3
├── delta-spark400db173/             # Databricks 17.3
└── delta-stub/                      # No-op fallback for unsupported profiles
```

The DB-14.3/DB-17.3 refactor adds one additional shared source root inside
`common/src/main/`:
...
Only `delta-spark350db143` and `delta-spark400db173` inject this source root.
...
Create a new Maven module following the same structure as `delta-spark350db143` (DBR
14.3), then dedupe the large overlap through the narrow shared
`common/src/main/db-350db143-400db173/scala` source root. After the 2026-05-06 refactor,
the DB-17.3 module contains only the remaining version-specific adapters and DB-17.3-only
files plus a `pom.xml`.
...
Present for module compatibility with the shared Databricks catalog plumbing. DB-17.3
CTAS/RTAS are tagged as CPU fallback in this PR; the command also defensively rejects
`WriteIntoDeltaEdge` so a catalog CTAS/RTAS path cannot silently run outside the GPU
transaction.
