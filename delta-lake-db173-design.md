...
**Review-comment sync + DV fallback hardening:** 2026-05-11 — see §0.K below.
**DML skip-row pushdown guard + DB assertion canary:** 2026-05-14 — see §0.L below.
**Issue 5 PR review/CI hardening:** 2026-05-21 — see §0.C.2 below.
**Status:** Issue 1 + Issue 2 + Issue 6 final DBR-17.3 native cuDF-only DV reads,
as well as Issue 8 + Issue 10 delivered. Issue 3 DELETE/UPDATE is in the base via
PR #14810; the smaller Issue 5 regular OPTIMIZE + inline auto-compaction PR is in
review as PR #14847 at `f8525abf7`; Issue 4 MERGE, Issue 5 liquid clustering,
Issue 7 CI, and Issue 9 identity-column work remain pending.
`delta_lake_write_test.py`, targeted `delta_lake_test.py`, targeted
`delta_lake_delete_test.py`, and the 2026-05-11 broader Delta read/delete plus
auto-compact selections are green on DB-17.3 for the validated local runs. The
2026-05-14 DML fallback regression is also green with the native-scan guard in
DV predicate pushdown, and the missing-row-index-filter assertion matcher now
has a DB wording canary in Python integration tests.
...
- **Deleted** (no shared-code reference forces the class to exist):
  ...
  - `GpuDoAutoCompaction.scala` (48 LOC) → returns in follow-up issue #14599;
    ...
    `GpuOptimisticTransaction.gpuWriteFiles`. The 2026-05-11 review-comment
    sync registers Databricks `AutoCompact` as a CPU post-commit hook instead.
...
Issue 5 (`delta_db173_OPTIMIZE_AUTO_COMPACT`) now reintroduces DBR 17.3 GPU
OPTIMIZE and auto-compaction in a clean review stack. The rewritten local commits
are `eda0aa172` (regular OPTIMIZE), `9ae539a3b` (auto-compaction), `0141241f0`
(liquid clustered OPTIMIZE / `OPTIMIZE FULL`), and `a05033865` (liquid
write-path completion). The regular OPTIMIZE + auto-compaction work should be
reviewed separately from the two liquid clustering commits.

The DBR 17.3 auto-compaction implementation uses a `GpuAutoCompact` post-commit
hook on the DBR 17.3 `CommittedTransaction` API rather than restoring the old
DBR 14.3-style `GpuDoAutoCompaction.scala` shape.

Latest Issue 5 PR-prep update: the smaller regular OPTIMIZE +
auto-compaction branch is `delta_db173_optimize_auto_compaction`; the May 21 review-fix tip is
`f8525abf7`, based on `origin/main` / `2ba5bc25e`. It includes the OPTIMIZE
commit `37903ee00`, the auto-compaction commit `704568255`, and follow-up fixes
for runtime writable-DV protection, DBR row-tracking commitInfo tag parity,
exception-safe inline auto-compaction observer cleanup, and DBR auto-compaction
min-file-size default behavior. Build passed and the focused
`delta_lake_optimize_table_test.py` + `delta_lake_auto_compact_test.py` run
passed with `19 passed`. Liquid clustering remains separate.
...
- Only `RowIndexFilterType.IF_CONTAINED` is supported on the native path; DBR
  row-index filters with other semantics, including `IF_NOT_CONTAINED`, fall back
  to CPU.
...
- Tagging also keeps row-index filter types other than `IF_CONTAINED` on CPU. This
  covers the `IF_NOT_CONTAINED` provider observed in DBR merge validation, whose
  keep-marked-row semantics are not expressible by the native cuDF DV drop-bitmap API.
- DV predicate pushdown removes the DBR skip-row predicate only under a child
  `GpuFileSourceScanExec` backed by `GpuDeltaParquetFileFormatNativeDV`. Plugin-enabled
  DML fallback plans, including DELETE plans that write persistent DVs on CPU, keep the
  skip-row filter.
...
- Targeted DBR DV read slice 2 passed: 120 selected, 108 passed, 12 expected Databricks
  skips.
- 2026-05-11 build passed again with
  `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh`.
...
`GpuOptimisticTransaction`, `GpuWriteIntoDelta`, the DML CPU-fallback stubs,
`GpuDeltaFileFormatWriter`, provider/probe files, and native DV read files.

### 0.K PR Review-comment Sync and DV Fallback Hardening (2026-05-11)

The `delta_db173_dv_native_ONLY_support` branch now carries the base PR review-comment
commit `362e9d346` below the amended native-only DV commit. Notable review-comment
follow-ups:

- `FilePartitionShims.copyWithFiles` clears `pathPrefix` after receiving files that
  were already expanded through `filesWithAbsolutePaths`, preventing a second relation-root
  application if the partition is processed again.
- The DB-17.3 wide-schema test-size reduction now names
  https://github.com/NVIDIA/spark-rapids/issues/14775 as the follow-up to restore normal
  coverage.
- CTAS/RTAS conversion is guarded in shared `DatabricksDeltaProviderBase`; DB-17.3 keeps
  CTAS/RTAS tagged to CPU and no longer carries local GPU conversion code for those execs.
- Databricks `AutoCompact` is registered as a CPU post-commit hook with RAPIDS temporarily
  disabled, preserving CPU auto-compaction behavior after GPU writes without reintroducing
  the deleted `GpuDoAutoCompaction` implementation.
- Shared `GpuCheckDeltaInvariant` wraps only the real check expression child. DBR 17.3's
  extractor children are used for CPU-side violation messages and are intentionally not
  converted.

The same branch also amends the native-only DV commit to tag non-`IF_CONTAINED` DBR
row-index filters for CPU fallback. The broadened validation after this hardening passed:
focused DV selection 194/194, `test_delta_merge_query` 1/1, broader
`delta_lake_test.py or delta_lake_delete_test.py` 289 tests with 40 skips, and
`delta_lake_auto_compact_test.py` 14/14.

### 0.L DML Skip-row Pushdown Guard (2026-05-14)

`DB173DVPredicatePushdown.pushToScan` now treats removal of DBR skip-row filters
as legal only for native GPU DV scans. The implementation checks the child plan
for a `GpuFileSourceScanExec` whose relation format is
`GpuDeltaParquetFileFormatNativeDV` before pruning the hidden skip-row column or
dropping the skip-row predicate.

This matters because DBR DELETE/DML bitmap-writing plans are still CPU fallback
paths for DB-17.3. They can appear while the plugin is enabled and can carry the
same skip-row predicate shape, but they need that predicate for Delta's DV
cardinality accounting. Removing it caused
`test_delta_delete_twice_with_dv` to fail with
`DELTA_DELETION_VECTOR_CARDINALITY_MISMATCH`. The fix is a plan-invariant guard,
not a narrow workaround for the test.

Validation after the guard:
- `WITH_DEFAULT_UPSTREAM_SHIM=0 ./jenkins/databricks/build.sh` passed.
- `delta_lake_delete_test.py::test_delta_delete_twice_with_dv` passed with
  seed `1778719081` and OOM injection enabled.
- `delta_lake_test.py::test_db173_missing_row_index_filter_assertion_guard`
  reflects DB's generated missing-row-index-filter assertion message and asserts
  RAPIDS' `MISSING_ROW_INDEX_FILTER_MESSAGE` still matches it. This is the
  integration-test replacement for a Scala test on DBR, where Scala tests are not
  run.
...
builds `delta-lake/delta-spark400db173`; writes, native cuDF DV reads, and
DELETE/UPDATE have GPU coverage in the current base. Regular OPTIMIZE + inline
auto-compaction is ready in the smaller `delta_db173_optimize_auto_compaction`
branch. Remaining follow-up areas are MERGE, liquid clustered OPTIMIZE /
`OPTIMIZE FULL`, CDC + DV reads, and CI enablement.
