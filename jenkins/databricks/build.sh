...
if [[ "$SCALA_BINARY_VER" == "2.13" ]]; then
    # For Scala 2.13, use scala2.13 module
    $MVN_CMD -B -f scala2.13/pom.xml -Ddatabricks -Dbuildver=$BUILDVER $MVN_PHASES -DskipTests $MVN_OPT
else
...
if [[ "$WITH_DEFAULT_UPSTREAM_SHIM" != "0" ]]; then
    ...
    if [[ "$SCALA_BINARY_VER" == "2.13" ]]; then
        ...
        $MVN_CMD -B -f scala2.13/pom.xml package -pl dist -am -DskipTests -Dmaven.scaladoc.skip $MVN_OPT \
