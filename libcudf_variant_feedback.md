...
## 7. Spark perspective — consolidated feedback

A narrative digest of everything above, written from the spark-rapids
consumer side. Read this section first if you have five minutes; drill into
§2–§6 for evidence and specifics. Every subsection points back to the
detailed sections that back it up.

### 7.1 The happy path — what we'd use unchanged

A spark-rapids query like `SELECT payload.user FROM events WHERE payload.clicks > 5`
on a VARIANT column can go GPU all the way down with this branch. The Parquet
reader materializes `payload` as `struct<list<uint8>, list<uint8>>` (§1); the
plan-side JSONPath parser splits a path like `payload.user.id` into segments;
each segment invokes `get_variant_field` via our JNI bridge, chained N times
for depth (validated by `ApacheNestedGetVariantField`); the leaf casts
through `cast_variant`. Null handling matches Spark exactly: missing field,
SQL NULL input, and variant-null primitive all collapse to SQL NULL (§0).

That's real value. Approximately half of Spark's `variant_get` surface
works with no further cuDF changes.

### 7.2 The silent failure — what breaks today without the cast-semantics fix

The other half is where adoption stops. `parse_json` picks the narrowest
integer encoding that fits, so `parse_json('{"clicks":5}')` stores `clicks`
as int8. Spark CPU returns `5` for `variant_get(v, '$.clicks', 'int')`. The
GPU path returns NULL — because `cast_variant(INT32)` is decode-if-exact-match
and int8 ≠ int32 (§3.6).

This is not a corner case. **Every integer field in every Variant column
produced by `parse_json` is a candidate for this failure.** The user sees
a column of NULLs where they expected values. No exception is thrown. No
warning fires. The spark-rapids fallback machinery doesn't catch it because,
from the plan's point of view, the GPU path "succeeded" — it just returned
NULL for every row.

This is the single most urgent finding in the feedback. Resolving it
(either by renaming `cast_variant` to reflect decode-if-match semantics
*and* adding a real cast API, or by extending `cast_variant` into a real
cast) is the P0 gate for spark-rapids adoption.

### 7.3 The adoption gates — what blocks production rollout

Even after cast semantics are fixed, three items still gate production use:

1. **Target-type expansion** beyond INT32/STRING (§2.1, §4.1). INT64 and
   DOUBLE alone cover most of the remaining Spark query surface; BOOLEAN,
   DECIMAL, TIMESTAMP, DATE round out the rest. Without these, queries fall
   back to CPU even when the cast semantics are right.
2. **Array-element access** (§2.2, §4.2). Spark users write paths like
   `'$.events[0].type'`; `get_variant_field` returns NULL for arrays today.
3. **`parse_json` on GPU** (§2.4). Ingest pipelines stay on CPU until this
   lands. This is where most Variant data is *created*, so it's the biggest
   ingest-path unlock.

From the Spark perspective these are three parallel workstreams, all
independent of each other and of the cast-semantics fix. Any of them can
land first.

### 7.4 The integration rough edges — fixable, worth fixing

Beyond the gates, four quality-of-life items that will bite consumers:

- **Name ↔ behavior mismatch on `cast_variant`** (§3.7). A function named
  `cast_variant` that doesn't cast is a code-review trap: reviewers trust
  the name, the bug ships, and the mismatch only manifests at runtime as
  silent nulls.
- **Null-mask-authoritative contract is in the implementation report, not
  the public header** (§3.4). A spark-rapids-jni developer who reads
  `cudf/io/variant.hpp` alone has no way to know the contract exists and
  will write buggy code that inspects masked-but-stale children.
- **Metadata deep-copy on chained calls** (§3.3). 4× metadata bandwidth
  on a 4-level nested path. Not a correctness issue, but measurable on
  schemas with long dictionaries.
- **Namespace placement in `cudf::io::parquet`** (§3.1). Variant is an
  in-memory Spark type too (the `VariantVal` produced by `parse_json`);
  the namespace suggests a storage-only scope that will age awkwardly.

### 7.5 The benchmark perspective — what the numbers tell us (and don't)

The published A100 numbers (§1 and the implementation report) establish:

- **Linear scaling above 128K rows** — kernel launch overhead is amortized
  at realistic Spark batch sizes. Good.
- **Dictionary scan is the hot path** — 2.4× spread from first-key to
  last-key on a 50-key dict (4.45 ms → 10.66 ms) is direct evidence that
  a `sorted_strings`-aware binary-search variant (§3.2) is the single
  highest-ROI optimization available without redesigning the algorithm.
- **Primitive decode is nearly free** — `cast_variant_int32` at 2M rows is
  0.099 ms. Useful calibration today, but this number will change
  significantly once real cast semantics land (string parsing on GPU isn't
  free). Plan to re-benchmark then.

What the numbers **do not** establish is what Spark users most want to
know: **is this faster than Spark's CPU path for realistic workloads?**
No CPU baseline is published (§6.2). We have a PySpark harness at
[`variant_prototype/cpu_benchmark/`](variant_prototype/cpu_benchmark/) that
measures the matching scenario × row-count × key-position matrix on Spark
4.0+; results will land in §6.3.1 once run on a representative machine.
Until that number exists, the value proposition is "probably much faster"
rather than "demonstrably N× faster" — a thinner argument internally.

Related benchmark gaps detailed in §6.2: end-to-end Parquet pipeline (not
just extraction kernel), heterogeneous row sizes (real Spark data is
Zipfian-skewed), newer GPUs (H100 and L40S are where new deployments
land), and multi-stream concurrency (a single Spark executor commonly
runs many concurrent kernels on one GPU).

### 7.6 The one-paragraph ask

From spark-rapids' perspective, **the cast-semantics decision on
`cast_variant` (§3.6 / Q0) is the single gate that matters most.** It
turns "approximately half of Spark's variant_get works" into "most of
Spark's variant_get works" for every workload the branch already covers
at the byte level. Every other item on our list — target-type expansion,
array access, `parse_json`, shredded read, the QoL fixes — is a parallel
workstream that can land independently, in any order. The branch's
architecture (two-pass kernel, `create_structs_hierarchy`, Apache
fixture validation) is sound and we want to consume it. The remaining
delta is a semantics clarification and a feature roadmap, not a rewrite.
